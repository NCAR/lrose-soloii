...
# soloii 1.18
#
# requires archive of source @ docker/lrose-soloii-$SOLOII_VERSION.tgz
# this can be handled by running ./docker/build-docker-soloii-1-18.sh
#
...
ENV SOLO_ARCHIVE /tmp/lrose-soloii.tgz
ENV SOLOII_VERSION 1.18
RUN mkdir $SOLO_DIR
COPY lrose-soloii-$SOLOII_VERSION.tgz $SOLO_ARCHIVE
RUN tar -xf $SOLO_ARCHIVE --directory $SOLO_DIR && rm -f $SOLO_ARCHIVE
