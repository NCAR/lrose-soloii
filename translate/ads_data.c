...
double gpro_decode(this, pct_samp, ardp)
  struct letvar_info *this;
  float pct_samp;
  struct ads_raw_data_point *ardp;
{
  ...
  struct ads_raw_data_point *ardp;
{
	...
	if(this->bits == 16 && this->skip == 0) {
	    ...
	    /* contiguous longs */
	    memcpy((char *)ardp->longs, a, this->sample*sizeof(long));
	}
	else {			/* assume interleaved longs */
	    inc = this->linc;
	    n = this->sample;
	    for(ll=(long *)a,i=0; i < n; ll+=inc)
		  ardp->longs[i++] = *ll;
	}
    }
}
...
struct ads_raw_data_que *gpro_sync(time)
  double time;
{
    /* c...mark */
