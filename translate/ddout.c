...
void ddout_headers_dump();
int  ddout_ini();
int  ddout_really();
void ddout_sweep_dump();
void ddout_sweeps_loop();
int  ddout_write();
void ddout_vol_data();
int  ddout_vol_detector();
void ddout_vols_loop();
...
int ddout_ini()
{
    ...
    char *dd_whiteout(), *dd_delimit(), *get_tagged_string();
    double lower;
	...
	if(a=get_tagged_string("DORADE_DEV")) {
	    ...
	    slash_path(str, dis->usi_top->directory);
	}
	...
	dd_file_name("dor", (long)lower, name, 0, str+strlen(str));
	if(IO_type == FB_IO)
	      strcat(str, ".tape");
	dfn = str;
    }
...
void ddout_sweeps_loop(time_span)
  int time_span;
{
    static int count=0;
    int ii, mark;
...
void ddout_sweep_dump(usi)
  struct unique_sweepfile_info_v3 *usi;
{
    static int count=0, loop_count;
    static double xtime=736902252;
    char *a, *b, *c, str[32], *str_terminate(), *dts_print(), *cc;
    struct dd_general_info *dgi, *dd_window_dgi();
	...
	printf("Number of parameters (%d) in %s/%s "
	       , dgi->num_parms, usi->directory, usi->filename);
    ...
    if(LittleEndian) {
       ddin_crack_swib(&tswib, swap_buf, (int)0);
       cc = swap_buf;
    }
    else { cc = (char *)&tswib; }
	...
	if(LittleEndian) {
	   ddin_crack_ryib(dds->ryib, swap_buf, (int)0);
	   cc = swap_buf;
	}
	...
	if(LittleEndian) {
	   ddin_crack_asib(dds->asib, swap_buf, (int)0);
	   cc = swap_buf;
	}
	...
	for(pn=0; pn < dgi->num_parms; pn++ ) {
	    ...
	    if(dgi->compression_scheme == NO_COMPRESSION) {
		  ...
		  mark = ddout_write(ddout_fid, dds->qdat[pn], ncopy);
		  mark = ddout_write(ddout_fid, dds->qdat_ptrs[pn]
				     , dds->qdat[pn]->pdata_length -ncopy);
	       }
	    }
	    else {
...
void ddout_vol_data(usi, time_span)
  struct unique_sweepfile_info_v3 *usi;
  int time_span;
{
    struct dd_general_info *dgi, *dd_window_dgi();
...
int ddout_vol_detector(volume_interval)
  int volume_interval;
{
    struct unique_sweepfile_info_v3 *usi;
    struct dd_general_info *dgi, *dd_window_dgi();
...
void ddout_vols_loop(volume_interval)
  int volume_interval;
{
    static int count=0;
    int ii, mark;
...
int ddout_hedr_saver( fid, buf, len )
  int fid, len;
  char *buf;
{
    int ii, nn, mark;
...
void ddout_headers_dump( usix, all )
  struct unique_sweepfile_info_v3 *usix;
  int all;
{
    ...
    char str[256], *str_terminate(), *dts_print(), *a, *get_tagged_string();
    DD_TIME xdts, *dts, *d_unstamp_time();
    long time_now();
    char *cc;
    ...
    if(LittleEndian) {
       ddin_crack_vold(vold, swap_buf, (int)0);
       cc = swap_buf;
    }
	...
	if(!usi->forget_it) {
		...
		for(jj=0; jj < dgi->num_parms; jj++ ) {
		   if(LittleEndian) {
		      ddin_crack_parm(dds->parm[jj], swap_buf, (int)0);
		      cc = swap_buf;
		   }
		...
		if(LittleEndian) {
		   ddin_crack_celv((char *)dds->celv, swap_buf, (int)0);
		   swack_long(&dds->celv->dist_cells[0], swap_buf +12
