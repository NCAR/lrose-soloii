...
# include <sys/time.h>
# include "uf_fmt.h"
/* "uf_fmt.h" is most likely found in the /rdss/radar/h directory */
...
static char *power_fields=POWER_FIELDS;
static char *velocity_fields=VELOCITY_FIELDS;
static char *toga_projects=TOGA_PROJECTS;
...
char **uf_print_dhed();
char **uf_print_fldh();
void uf_print_headers();
char **uf_print_lus();
char **uf_print_lus_ac();
char **uf_print_lus_gen();
char **uf_print_lus_ucla();
char **uf_print_man();
char **uf_print_opt();
void uf_print_stat_line();
void uf_rename_str();
void uf_reset();
int uf_select_ray();
double uf_time_stamp();
void uf_vol_update();
  ...
  int rdn;
{
    ...
    if(LittleEndian) {
       uf_crack_loc_use_ncar (loc, xloc, (int)0);
       lus = xloc;
    }
...
void ucla_ac_header()
{
    ...
    if(LittleEndian) {
       uf_crack_loc_use_ucla (loc, xloc, (int)0);
...
void ufx_ini()
{
    ...
    char *a, *aa, *get_tagged_string(),  *buf = NULL;
...
# endif
    ...
    dd_establish_source_dev_names("UF", buf);
    aa = dd_next_source_dev_name("UF");
...
int uf_next_ray()
{
    ...
    char radar_name[12], *str_terminate(), *aa;
    int i, ii, n, nn, rdn, eof_count=0, err_count=0, new_vol=NO, size = -1;
    double uf_time_stamp(), del;
    DD_TIME *dts=gri->dts;
...
Option = "
	...
	uf_print_headers(irq->top->buf, pbuf_ptrs);
    }
    else if(ival == 12) {
	uf_print_data(irq->top->buf, pbuf_ptrs);
    }
    else if(ival == 14) {
    }
    preamble[0] = '\0';
...
char **
uf_print_data(uf, ptrs)
  short *uf;
  char **ptrs;
{
    int gg, ii, nf, ngx=0;
    short *ss, *dh;
    char ts[8], *str_terminate();
    char **pt=ptrs;
    float unscale[7], ff;
    short *flds[7];
    int ng[7];
  ...
  char **ptrs;
{
    ...
    char *str_terminate();
    char **pt=ptrs;
    char **uf_print_lus_gen(), **uf_print_lus_ac(), **uf_print_lus_ucla();
    char *aa = (char *)loc;
...
# endif
	...
	pt = uf_print_lus_ucla(loc, ptrs);
    }
    else {
       if(LittleEndian) {
	  swack_short(loc, xloc, len);
	  pt = uf_print_lus_gen(xloc, lundx, len, ptrs);
       }
       else {
	  pt = uf_print_lus_gen(loc, lundx, len, ptrs);
       }
    }
    return(pt);
}
...
char **
uf_print_lus_gen(lus, lundx, len, ptrs)
  short *lus;
  int lundx, len;
  char **ptrs;
{
  ...
  char *a;
{
	...
	uui->top_ren->last = this;
	this->next = uui->top_ren;
	last = this;
    }
}
/* c------------------------------------------------------------------------ */
