...
static unsigned long P960501=0;
static unsigned long P970101=0;
static unsigned long P970127=0;
static unsigned long P970129=0;
static unsigned long P980201=0;
...
int fof_sl_next_entry();
void piraq_dd_conv();
int piraq_isa_new_vol();
int piraq_isa_new_sweep();
int piraq_select_ray();
void piraq_print_stat_line();
void piraq_reset();
static void piraq_ini();
void piraq_name_replace();
void piraq_name_aliases();
static int piraq_next_ray();
static void piraq_backwards_reset();
static void piraq_map_hdr();
static void piraq_header();
static void piraq_fields();
static void simplepp();
double pc_swap4f();
long pc_swap4();
void piraq_positioning();
int piraq_inventory();
void piraq_dump_this_ray();
void piraq_print_headers();
void piraq_print_hdr();
void piraq_print_rhdr();
...
void dualspolx();
int pui_next_block();
static void do_Noise_lut12();
static void do_Noise_lut15();
...
static void do_Noise_lut12( noise, scale2ln, lut )
    double noise;
    double scale2ln;
    float * lut;
{
...
static void do_Noise_lut15( noise, scale2ln, lut )
    double noise;
    double scale2ln;
    float * lut;
{
...
static void do_Noise_lut( noise, scale2ln )
    double noise;
    double scale2ln;
{
    int ii;
...
{
    static int ebug = 0;
    int ii, nn, new = NO, az_turnaround, el_turnaround;
    int cond_a, cond_b, cond_c;
    int lag_rays = 2;
    double mrd, d, d2, d3, angdiff(), ang_tol, time_tol, dt, da;
...
{
    int ii, jj, nn, nt;
    CHAR_PTR aa, bb, put_tagged_string(), get_tagged_string();
    ...
    dts.month = 5;
    dts.day = 1;
    P960501 = d_time_stamp(&dts);

    dd_clear_dts(&dts);
    dts.year = 1997;
    dts.month = 1;
    dts.day = 1;
    P970101 = d_time_stamp(&dts);

    dts.year = 1997;
    dts.month = 1;
    dts.day = 27;
    P970127 = d_time_stamp(&dts);

    dts.year = 1997;
    dts.month = 1;
    dts.day = 29;
    P970129 = d_time_stamp(&dts);

    dts.year = 1998;
    dts.month = 2;
    dts.day = 1;
    P980201 = d_time_stamp(&dts);
...
# endif
	...
	bb = dd_establish_source_dev_names("PRQ", aa);
    }
    if( aa && bb ) {
      current_file_name = dd_next_source_dev_name("PRQ");
...
void piraq_name_replace(name, top)
  char *name;
  struct rename_str *top;
{
    struct rename_str *this=top;

    for(; this; this = this->next) {
	if(strstr(name, this->old_name)) {
	    strcpy(name, this->new_name);
	    break;
	}
    }
}
...
void piraq_name_aliases(aliases, top)
  char *aliases;
  struct rename_str **top;
{
    /* routine to construct the list of aliases */
    struct rename_str *this, *last;
	...
	this = (struct rename_str *)malloc(sizeof(struct rename_str));
	memset(this, 0, sizeof(struct rename_str));

	/* get the first name */
	strcpy(this->old_name, str_ptrs[ii]);

	/* get the second name */
	strcpy(this->new_name, str_ptrs[ii+2]);
...
piraq_next_ray()
{
    ...
    struct io_packet *dd_return_next_packet(), *dp, *dpx;
...
piraq_header()
{
    ...
    char *aa, *str_terminate(), mess[256];
...
static void 
simplepp(DWELL *dwell, RADARV *radar, float *prods)      
{
    int mark, gg;
    int  i;
    float        *aptr,*pptr;
...
static void 
simplepp16(DWELL *dwell, RADARV *radar, float *prods)      
{
...
# endif
	...
	*velp++ = DD_SCALE((flip_vels ? -f : f), scale, bias);
...
void piraq_positioning()
{
    ...
    float f;
    double d, fabs(), dd_relative_time(), dtarget, d_time_stamp();
...
Option = "
	...
	if(gri_start_stop_chars(&view_char, &view_char_count) >= 0) {
		...
		Xctypeu16(irq->top->buf, view_char, view_char_count);
	    }
	    else {
  ...
  struct solo_list_mgmt *slm;
{
    ...
    char *aa, *bb, *cc, str[128], *str_terminate(), tmp_str[16];
...
{
    int ii;
    char *aa, str[128], *str_terminate(), tmp[32];
...
{
    int ii, jj, nc, nn;
    char *aa, *bb, *cc, *zz, str[128], *str_terminate(), tmp[32];
...
# else
	...
	*v1p++ = DD_SCALE(f, scale, bias);
	f = v2 * ratio * velconst;           /* velocity short PRT in m/s */
	*v2p++ = DD_SCALE(f, scale, bias);
# endif
...
void dualprt(DWELL *dwell, RADARV *radar, float *prods)      
   {
   int  i;   
...
# endif
      *dbmv++ = DD_SCALE(f, scale, bias); /* vertical power "DL" */
...
void dualpol12(DWELL *dwellness, RADARV *radar, float *prods) {
	...
	*rhohv++ = DD_SCALE(d, gri->dd_scale[rhohv_ndx]
			    , gri->dd_offset[rhohv_ndx]);

	if( dwel->dataformat == DATA_POL12 && pui->options & FULL_MATRIX ) {
	    /* cross correlation rho horizontal */
	    d = exp( h_rho -0.5 * ( lnhv + lnph ));
	    *hrho++ = DD_SCALE(d, scale, bias);
	    *hang++ = DD_SCALE(DEGREES( h_ang ), scale, bias);
	    
	    
	    /*	// cross correlation rho vertical	 */
	    d = exp( v_rho -0.5 * ( lnhv + lnpv ));
	    *vrho++ = DD_SCALE(d, scale, bias);
	    *vang++ = DD_SCALE(DEGREES( v_ang ), scale, bias);
	}
# ifdef obsolete
...
# endif
	...
	*dbmp++ = DD_SCALE(horiz_dBm_at_coupler, scale, bias);
...
# endif
	...
	*dbmv++ = DD_SCALE(d, scale, bias); /* vertical power "DL" */
	...
	*dbmx++ = DD_SCALE(d, scale, bias); /* cross pole power "DX" */
    }
}
/* c------------------------------------------------------------------------ */
#define LOG2            0.69314718056
...
# endif
	...
	if( pui->options & FULL_MATRIX ) {
	    ...
	    *hrho++ = DD_SCALE(d, scale, bias);
	    *hang++ = DD_SCALE(DEGREES( vcrhv ), scale, bias);
	    
	    
	    /*	// cross correlation rho vertical	 */
	    d = exp( lncrvh -0.5 * ( lnpv + lnvh ));
	    *vrho++ = DD_SCALE(d, scale, bias);
	    *vang++ = DD_SCALE(DEGREES( vcrvh ), scale, bias);
	}
...
# endif
	...
	if( pui->options & FULL_MATRIX ) {
	    ...
	    *iq_norm++ = DD_SCALE(d, scale, bias);
	    *iq_ang++ = DD_SCALE(DEGREES( phiq ), scale, bias);
	}
...
# endif
	...
	if( pui->options & FULL_MATRIX ) {
	    ...
	    *ldrv++ = DD_SCALE(d, scale, bias);	/* Ldr Vertical */
	}
	/* this space intentionaly left blank */
	/* RHOhv is computed above, before noise correction */
...
# else
	*dbmv++ = DD_SCALE(verti_dBm_at_coupler, scale, bias); /* vertical power "DL" */
# endif
