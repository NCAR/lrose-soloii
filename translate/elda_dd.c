...
void eld_dd_conv();
void cspd2celv();
int dde_desc_fwd_sync();
void dor_print_flib();
void dor_print_lidr();
void eld_dump_raw_header();
void ddin_dump_this_ray();
void eldx_ini();
void eld_init_sweep();
void eld_interest();
int eld_inventory();
int eld_is_new_sweep();
void eld_lidar_avg();
void eld_lut();
int eld_next_ray();
...
void eld_print_frad();
void eld_print_ldat();
void eld_print_stat_line();
void eld_radar_rename();
void eld_reset();
struct eldora_unique_info *eld_return_eui();
int eld_select_ray();
int eld_stuff_data();
void eld_stuff_ray();
void eld_thr_flds();
...
void eld_dd_conv(interactive)
  int interactive;
{
    int i, n, mark, rn;
    long tn, time_now();
    char str[256];
    static count=0, count_rejects=0, count_break=6000;
...
int dde_desc_fwd_sync(aa, ii, jj)
    char *aa;
    int ii, jj;
{
...
void
dor_print_flib(flib, slm)
  struct field_lidar_i *flib;
...
void
eld_dump_raw_header(dgi, irq, vol_dp, slm)
  struct dd_general_info *dgi;
  struct input_read_info *irq;
  struct io_packet *vol_dp;
  struct solo_list_mgmt *slm;
{
...
void
eld_dump_this_ray(dgi, slm)
  struct dd_general_info *dgi;
  struct solo_list_mgmt *slm;
{
    int ii, gg, np=dgi->source_num_parms, sizeof_data;
    char *b;
    char ts[12], *str_terminate();
    short *flds[7];
...
void eldx_ini()
{
    ...
    char *a, *aa, *bb, *cc, *dd, *get_tagged_string();
    char string_space[2048], *str_ptrs[256], str[16];
    double d, d1, d2, eldora_time_stamp();
	...
	eui->time_correction = -(eui->julian_day_bias*SECONDS_PER_DAY);
     ...
     */
    bb = "01/01/95:00:00"; dd_crack_datime(bb, strlen(bb), &dts);
    time_for_nimbus = start_vortex_fix_1 = d_time_stamp(&dts);

    bb = "04/01/95:00:00"; dd_crack_datime(bb, strlen(bb), &dts);
    end_vortex_tests = d_time_stamp(&dts);

    bb = "04/22/95:00:00"; dd_crack_datime(bb, strlen(bb), &dts);
...
# ifdef obsolete
     ...
     */
    for(this=dgi->swp_que->last,ii=0; ii < MAX_PREV_SWPS; ii++) {
	if(this->source_sweep_num == ryib->sweep_num) {
	    break;
	}	      
	this = this->last;
    }
...
void eld_interest(dgi, verbosity, preamble, postamble)
  struct dd_general_info *dgi;
  int verbosity;
  char *preamble, *postamble;
{
     ...
     */
    int ii;
    char str[256], *dts_print(), *str_terminate();
...
int eld_is_new_sweep(dgi)
  struct dd_general_info *dgi;
{
    int ii, new=NO, rn=eui->radar_num;
    double angdiff();
    float ra, rb;
    struct dds_structs *dds=dgi->dds;
    struct individual_radar_info *iri;
  ...
  DGI_PTR dgi;
{
    ...
    long *sums, *lp, *lpzz;
    double rcp_num;
    struct running_avg_que *raq, *se_return_ravgs();
    struct que_val *qv;
	...
	lp = iri->green_sums = (long *)malloc(nn);
	memset(lp, 0, nn);
	lp = iri->red_sums = (long *)malloc(nn);
    	memset(lp, 0, nn);
	...
	switch(loop) {
    	case 0:
	    pn = dd_find_field(dgi, "GREEN");
	    sums = iri->green_sums;
	    raq = iri->green_raq;
	    break;
	case 1:
	    pn = dd_find_field(dgi, "INFRARED");
	    sums = iri->red_sums;
	    raq = iri->red_raq;
	    break;
	}
	...
	if(doit) {
		...
		*us++ = rcp_num * (*lp++);
	    ...
	    for(; gg++ < nc; *us++ = bad);
	    memset(sums, 0, eui->max_cells * sizeof(long));
	}
    }
}
/* c------------------------------------------------------------------------ */

void eld_lut(dgi, pn, dst)
  DGI_PTR dgi;
  int pn;
  short *dst;
{
    ...
    char *aa, name[16], *str_terminate();
    double atof(), ts=dgi->time, nyqv;
    
    bias = parm->parameter_bias;
    rcp_scale = 1./parm->parameter_scale;
    aft = strstr(dgi->radar_name, "TA") != NULL;


    if(eui->options AND TOGACOARE_FIXES &&
    ...
    else if(eui->options AND TOGACOARE_FIXES &&
...
# else
		*ss++ = DD_SCALE((float)i, scale, (float)0);
# endif
	    }
	}
	else {
...
int eld_next_ray()
{
    ...
    int ignore_cfacs = NO;
    char *aa, *at, *b, radar_name[12], *str_terminate(), dname[16], *dst;
    char *dts_print(), str[256], frib_fname[80], *cfac_id;
    DD_TIME *d_unstamp_time();
    char *deep6=0;
    ...
    struct time_dependent_fixes *fx, *fx_kill, *dd_clean_tdfs();
	...
	/* c...mark */
	   ...
	   if(*eld_next_block == 'L') {
	      if(LittleEndian) {
		 eld_crack_ldat(eld_next_block, xldat, (int)0);
		 ldat = xldat;
	      }
	      else
		...
		if(ldat) {
		   if(LittleEndian) {
		      eld_crack_ldat(eld_next_block, dds->ldat, (int)0);
		   ...
		   if(LittleEndian) {
		      eld_crack_frad(eld_next_block, dds->frad, (int)0);
		      swack_short(eld_next_block + sz, dds->raw_data
				  , (gd->sizeof_struct -sz)/sizeof(short));
		   }
		   else {
	     ...
	     */
	   if(LittleEndian) {
	      ddin_crack_ryib(ryib, dds->ryib, (int)0);
	   }
	   else {
	    ...
	    eui->current_time = eui->prior_time = dgi->time = dts->time_stamp;

	   if(LittleEndian) {
	      ddin_crack_asib(asib, dds->asib, (int)0);
	   }
	   else {
...
# endif
	    ...
	    if(LittleEndian) {
	       ddin_crack_parm(eld_next_block, dds->parm[pn], (int)0);
	    }
	    else {
	   ...
	   lidr = NULL;
	    ...
	    if(lidr) {
	       if(LittleEndian) {
		  eld_crack_lidr(eld_next_block, dds->lidr, (int)0);
	       }
	       else {
		...
		if(eui->data_type & NAILS_DATA) {
		    radd->radar_type = LIDAR_FIXED;
		}
		else {
		    radd->radar_type = LIDAR_MOVING;
		}
		...
		radd->num_freq_trans = lidr->num_wvlen_trans;
	       ...
	       if(LittleEndian) {
		  ddin_crack_radd(eld_next_block, dds->radd, (int)0);
	       }
	       else {
		  memcpy((char *)dds->radd, eld_next_block
	       ...
	       /* klooges!
		*/
	       if(eui->options AND TOGACOARE_FIXES && aft) {
	   ...
	   if(!ignore_cfacs) {
	      if(LittleEndian) {
		 ddin_crack_cfac(eld_next_block, dds->cfac, (int)0);
	      }
	      else {
	   ...
	   if(LittleEndian) {
	      eld_crack_cspd(eld_next_block, xcspd, (int)0);
	      cspd = xcspd;
	   }
	   else {
...
# endif
	    ...
	    if(LittleEndian) {
	       ddin_crack_vold(eld_next_block, xvold, (int)0);
	       vold = xvold;
	    }
	    else {
...
void eld_print_stat_line(count, dgi)
  int count;
  struct dd_general_info *dgi;
{
...
void eld_reset()
{
    ...
    int ii;
    struct individual_radar_info *iri;
    struct dd_general_info *dgi, *dd_get_structs();
...
int eld_stuff_data( dgi, parm_num)
  struct dd_general_info *dgi;
  int parm_num;
{
    char *dst;
...
void eld_stuff_ray(dgi, current_time)
  struct dd_general_info *dgi;
  time_t current_time;
{
    /*
    ...
    char *b, str[256], *dts_print();
    static int count=0, zit=123;
    double d,  eldora_time_stamp();
    double dd_rotation_angle();
    DD_TIME *d_unstamp_time(), *dts;
    ...
    if(dgi->new_sweep) {
	if(eui->options AND ELD_AC_COMPARE) {
	...
	eld_nimbus_fix_asib(dgi->dds->dts, asib, eui->options, dgi->radar_num);
    }
    else {
