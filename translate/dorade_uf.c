  ...
  struct dd_general_info *dgi;
{
	...
	if((a=get_tagged_string("UF_DEV"))) {
	   ...
	   uod->dev_name = dgi->directory_name;
	}
  ...
  struct uf_production *ufp;
{
    ...
    int ii, m, n, nt;
    char *uf_ptr, str[512], *a, *ufn, *get_tagged_string();
    ...
    else if( radd->radar_type != GROUND ) {
  ...
  struct uf_production *ufp;
{
    ...
    if(dgi->new_sweep) {
       ...
       man->fixed_angle_x64 = S64(swib->fixed_angle);
    ...
    man->altitude_msl = dd_altitude(dgi) * 1000.;
    man->azimuth_x64 = S64(dd_azimuth_angle(dgi));
    man->elevation_x64 = S64(dd_elevation_angle(dgi));

    man->fixed_angle_x64 = S64(swib->fixed_angle);
    man->sweep_rate_x64 = S64(ryib->true_scan_rate);
	...
	luac->ac_pitch_x64 = S64(dd_pitch(dgi));
	luac->ac_roll_x64 = S64(dd_roll(dgi));
	luac->ac_phdg_x64 = S64(dd_heading(dgi));
	luac->ac_drift_x64 = S64(dd_drift(dgi));
	d = FMOD360( asib->rotation_angle + dgi->dds->cfac->rot_angle_corr);
	luac->ac_rotation_x64 = S64(d);
	d = FMOD360( asib->tilt + dgi->dds->cfac->tilt_corr);
	luac->ac_tilt_x64 = S64(d);
	luac->ac_phdg_change_x64 = S64(asib->heading_change);
	luac->ac_pitch_change_x64 = S64(asib->pitch_change);
	luac->ac_ant_rot_angle_x64 = S64(dd_rotation_angle(dgi));
	luac->ac_tilt_angle_x64 = S64(dd_tilt_angle(dgi));

	luac->ac_vns_x10 = S10(asib->ns_velocity);
	luac->ac_vew_x10 = S10(asib->ew_velocity);
	luac->ac_wp3_x10 = S10(asib->vert_velocity);
	luac->ac_ui_x10 = S10(asib->ns_horiz_wind);
	luac->ac_vi_x10 = S10(asib->ew_horiz_wind);
	luac->ac_wi_x10 = S10(asib->vert_wind);
    }
	...
	if( dgi->parm_type[pn] == VELOCITY ) {
	    fhed->word_20 = UF_SCALE(radd->eff_unamb_vel, scale);
	    strncpy((char *)&fhed->word_21, "  ", 2 );
	}
	else {
	    fhed->word_20 = man->missing_data_flag;
	    fhed->word_21 = UF_SCALE(radd->noise_power, scale);
	    fhed->word_22 = UF_SCALE(radd->receiver_gain, scale);
	    fhed->word_23 = UF_SCALE(radd->peak_power, scale);
	    fhed->word_24 = UF_SCALE(radd->antenna_gain, scale);
	    fhed->word_25 =
		  S64(1.e6*dds->parm[pn]->pulse_width/SPEED_OF_LIGHT);
	}
  ...
  struct uf_production *ufp;
{
    ...
    fhed->h_beam_width_x64 = S64(radd->horz_beam_width);
    fhed->v_beam_width_x64 = S64(radd->vert_beam_width);
    fhed->rec_bw_xe6 = dds->parm[0]->recvr_bandwidth;
    fhed->polarization = dds->parm[0]->polarization;
    fhed->waveln_x6400 = S64(100.*SPEED_OF_LIGHT/(radd->freq1*1.e9));
    fhed->num_samples = dds->parm[0]->num_samples;
    fhed->prT_xe6 = radd->interpulse_per1*1.e6;
    ...
    fhed->gate_spacing_xe3 = ufp->uf_gate_spacing;
    fhed->gt_sp_adjust_xe6 =
	  S1000(ufp->uf_gate_spacing-fhed->gate_spacing_xe3);
  ...
  int size;
{
	...
	*/
	  ...
	  swack2(&fidx->fld_hed_ptr, ss); ss++;

	  ii = FsCs(fidx->fld_hed_ptr);
	  fh = (UF_FLD_HED *)(ufp->uf_buf + ii);
	  uf_crack_fhed(fh, le_buf + ii, (int)0);

	  ii = FsCs(fh->fld_data_ptr);
	  swack_short(ufp->uf_buf + ii, le_buf + ii, (int)fh->num_gates);
       }
       buf = le_buf;
    }
    ...
    if( UF_null_dev )
      ...
      { ii = gp_write( ufp->uf_fid, buf, size, ufp->io_type ); }
    return(ii);
}
/* c------------------------------------------------------------------------ */
