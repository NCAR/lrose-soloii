...
static long dd_start_time=0, dd_stop_time=ETERNITY;
...
DGI_PTR dd_get_structs(), dgi_last();
int ddin_ini();
int ddin_next_ray();
...
process_cappi_beam() { int i=0; } /* KLOOGE! */ 

/* c------------------------------------------------------------------------ */

void solo_message(message)	/* some of the access routines call
				 * this routine
				 */
  char *message;
{
...
void dap_givfld_( ndx, g1, n, dd, badval )
  int *ndx, *g1, *n;
  float *dd, *badval;
{
...
int dap_mnt_f_( input_file_name, radar_name, start_time, stop_time
	   , ivol, iscan, irec )
  char *input_file_name, *radar_name;
  ...
  int *ivol, *iscan, *irec;
{
    ...
    */
    ...
    if(first) {
	first = NO;
	put_tagged_string("SOURCE_DEV", input_file_name);
	if(ddin_ini() < 0) {	/* from ddin.c */
	   ierr = ACCESS_FAILURE;
	   exit(1);
	}
    }
...
int dap_mnt_s_( dir, radar_name, start_time, stop_time
	   , ivol, iscan, irec )
  char *dir, *radar_name;
  long *start_time, *stop_time;
  int *ivol, *iscan, *irec;
{
    ...
    DGI_PTR dd_window_dgi();
    struct dd_general_info *dd_ini();

    put_tagged_string("DORADE_DIR", dir);
    put_tagged_string("SELECT_RADARS", radar_name);
...
void dap_range_gate_( req_range, gate, val )
  int *gate;
  float *req_range, *val;
{
  ...
  char *buf;
{
	...
	mark = lseek(fid, 0L, 1L); /* mark offset */
	if((n=read( fid, buf, BLOCK_SIZE )) <
	   sizeof(struct generic_descriptor)) {
	    return(n);
	}
	...
	mark = lseek(fid, mark+k, 0L); /* move offset to last ray boundary */
	return(k);
    }
...
void dap_rng_info_( g1, m, rngs, ng )
  int *g1, *m, *ng;
  float *rngs;
{
...
int dap_next_ray_loop()
{
	...
	if(dd_radar_num < 0 ) {
	    ...
	    if(strstr(dgi->radar_name, local_radar_name)) {
...
DGI_PTR dap_another_ray()
{
    ...
    char *a, *b, *c, str[32], *dts_print();
    double eldora_time_stamp(), fmod();
...
/* c------------------------------------------------------------------------ */
int kday_() { return(dgi->dds->vold->day); }
/* c------------------------------------------------------------------------ */

void kfld1_( buf, name2 )
  char *buf, *name2;
{
...
/* c------------------------------------------------------------------------ */
int khour_() { return(dgi->dds->ryib->hour); }
/* c------------------------------------------------------------------------ */
int klrect_() { return( dgi->ray_num ); }
/* c------------------------------------------------------------------------ */
int kminit_() { return(dgi->dds->ryib->minute); }
/* c------------------------------------------------------------------------ */
int kminut_() { return(dgi->dds->ryib->minute); }
/* c------------------------------------------------------------------------ */
int kmonth_() { return(dgi->dds->vold->month); }
/* c------------------------------------------------------------------------ */
int kprnv_() { return(dgi->ray_num); }
/* c------------------------------------------------------------------------ */

void kproj_( buf, name8 )
  char *buf, *name8;
{
...
void kradar_( buf, name8 )
  char *buf, *name8;
{
    strncpy( name8, dgi->dds->radd->radar_name, 8 );
}
/* c------------------------------------------------------------------------ */
int ksecnd_() { return(dgi->dds->ryib->second); }
/* c------------------------------------------------------------------------ */

void ksite_( buf, name8 )
  char *buf, *name8;
{
    strncpy( name8, "ELECTRA   ", 8 );
}
/* c------------------------------------------------------------------------ */
int kswepm_() { return(dgi->dds->radd->scan_mode); }
/* c------------------------------------------------------------------------ */
int kswepn_() { return(dgi->sweep_count); }
/* c------------------------------------------------------------------------ */

void ktapen_( buf, name8 )
  char *buf, *name8;
{
    strncpy( name8, "UNKNOWN ", 8 );
}
...
void ktimez_( yy, mo, dd, hh, mm, ss, ms )
  int *yy, *mo, *dd, *hh, *mm, *ss, *ms;
{
    int y = dgi->dds->vold->year;
...
void ktzone_( buf, name2 )
  char *buf, *name2;
{
    strncpy( name2, "UTC", 2 );
}
/* c------------------------------------------------------------------------ */
int kvoln_() { return(dgi->vol_num); }
/* c------------------------------------------------------------------------ */
int kyear_() {int y = dgi->dds->vold->year; return(y > 1900 ? y-1900 : y); }
/* c------------------------------------------------------------------------ */
void cuazim_(x)
  float *x;
{
    double d, fmod(), dd_azimuth_angle();
  ...
  float *x;
{
	...
	*x = dgi->dds->asib->rotation_angle;
    ...
    else if( dgi->dds->radd->scan_mode == RHI ) {
...
void dap_rng_info__( g1, m, rngs, ng )
  int *g1, *m, *ng;
  float *rngs;
{
...
void dap_givfld__( ndx, g1, n, dd, badval )
  int *ndx, *g1, *n;
  float *dd, *badval;
{
    dd_givfld( dgi, ndx, g1, n, dd, badval );
...
void dap_range_gate__( req_range, gate, val )
  int *gate;
  float *req_range, *val;
{
...
int dap_mnt_s__( dir, radar_name, start_time, stop_time
	   , ivol, iscan, irec )
  char *dir, *radar_name;
  long *start_time, *stop_time;
  int *ivol, *iscan, *irec;
...
int dap_mnt_f__( input_file_name, radar_name, start_time, stop_time
	   , ivol, iscan, irec )
  char *input_file_name, *radar_name;
  long *start_time, *stop_time;
  int *ivol, *iscan, *irec;
