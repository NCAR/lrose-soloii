...
void toga_data_types();
void toga_dd_conv();
void toga_gen_luts();
void toga_grab_header_info();
void toga_ini();
void toga_nab_data();
void toga_nab_info();
int toga_next_ray();
int toga_select_ray();
int toga_ok_ray();
void toga_print_stat_line();
double toga_time_stamp();
void toga_update_header_info();
...
void toga_gen_luts()
{
    ...
    *lut++ = EMPTY_FLAG;
    ...
    for( i=0; i < 256; i++ )
	 *lut++ = S100(((float)i)*RCP512*2.0*nyq);
    /*
...
void toga_ini( start_time, stop_time, dir, toga_file, project_name
	 , site_name, output_flags)
  long start_time, stop_time;
  char *dir, *toga_file, *project_name, *site_name, *output_flags;
{
    /* c...mark */
    int i, j, n;
    char *c, *put_tagged_string();
...
void toga_print_stat_line(count)
  int count;
{
    char *dts_print();
...
double toga_time_stamp( trah, dts )
  struct toga_ray_header *trah;  
  DD_TIME *dts;
{
