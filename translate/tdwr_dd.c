...
void tdwr_dd_conv();
void tdwr_dump_this_ray();
void tdwr_ini();
int tdwr_inventory();
void tdwr_map_structs();
void tdwr_nab_data();
void tdwr_nab_dataLE();
int tdwr_next_ray();
void tdwr_positioning();
void tdwr_print_cbdh();
void tdwr_print_stat_line();
void tdwr_reset();
int tdwr_select_ray();
void tdwr_setup_fields();


/* c------------------------------------------------------------------------ */

void tdwr_dd_conv(interactive_mode)
...
void tdwr_ini()
{
    ...
    char str[256], line[128], directory[128], *str_terminate();
    char *a, *aa, *bb, *get_tagged_string(), *unk="UNKNOWN";
	...
	aa = ii == LOW_RATE_INDEX ? "TDWR_LR" : "TDWR";
    ...
    for(ii=0; ii < 256; ii++) {
	*ss++ = DD_SCALE(TDWR_UNSCALE(ii, Z_UNSCALE, Z_EXCESS), 100., 0);
    }
    ...
    for(ii=0; ii < 256; ii++) {
	*ss++ = DD_SCALE(TDWR_UNSCALE(ii, SNR_UNSCALE, SNR_EXCESS), 100., 0);
    }
    ...
    for(ii=0; ii < 256; ii++) {
	*ss++ = DD_SCALE(TDWR_UNSCALE(ii, SW_UNSCALE, SW_EXCESS), 100., 0);
    }
	...
	*ss++ = DD_SCALE(TDWR_UNSCALE(ii, V_UNSCALE, V_EXCESS), 100., 0);
    }
  ...
  char *buf;
{
   ...
   if(LittleEndian) {
      cbdh = xcbdh;
      tdwr_crack_cbdh(buf, cbdh, (int)0);
      ul = cbdh->pw0; 
...
int tdwr_next_ray()
{
	...
	case TDWR_SUN_TRACK:
	    ...
	    break;
	default:
	    gri->scan_mode =  pw2.lp ? SUR : PPI; /* low prf scan? */
	    break;
	}
...
void tdwr_print_stat_line(count)
  int count;
{
