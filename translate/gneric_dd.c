...
void dd_init_sweep(dgi,new_vol)
  struct dd_general_info *dgi;
  int new_vol;
{
    CHAR_PTR dd_radar_name(), dd_blank_fill();
    short date[6];
    time_t todays_date();
    int i;
    ...
    for(ipps= &dds->radd->interpulse_per1,i=0; i < gri->num_ipps; i++)
	  *ipps++ = gri->ipps[i];
    strcpy(dgi->source_field_mnemonics, gri->source_field_mnemonics);
}
/* c------------------------------------------------------------------------ */

void dd_parms_update(dgi)
  struct dd_general_info *dgi;
{
    int ii, jj, ncopy;
    char *str_terminate();
	...
	dds->parm[ii]->pulse_width = .5 * gri->pulse_width * SPEED_OF_LIGHT;
...
void dd_radd_update(dds)
  struct dds_structs *dds;
{
    int i;
    struct radar_d *radd=dds->radd;
    CHAR_PTR dd_blank_fill();
    FLOAT_PTR freq, ipps;
...
void dd_stuff_ray()
{
    ...
    float elev=0;
    double eldora_time_stamp(), dorade_time_stamp(), angdiff();
    ...
    dgi = dd_get_structs(gri->dd_radar_num, gri->radar_name);
