  ...
  struct eldora_unique_info *eui;
{
    ...
    thrv = DD_SCALE(eui->ncp_threshold, dds->parm[tf]->parameter_scale
	...
	*ss++ = 1;
    ...
    if(eui->options AND ELD_DESPECKLING) {
...
eld_tweak_vel(eui, fn, dst, fgg, ok_options)
  struct eldora_unique_info *eui;
  int fn, fgg, ok_options;		/* field num and first good gate */
  short *dst;
{
    ...
    double d, atof(), fmod(), fabs(), sin(), cos(), atan(), sqrt();
    ...
    double cosPHI, cosLAM, sinPHI, sinLAM, angdiff();
    int nc=dgi->clip_gate+1;
	...
	if(!dgi->ray_que->last->data[fn]) {
	    ...
	    dgi->ray_que->data[fn] = (long *)
		  malloc(MAXCVGATES*sizeof(long));
	}
	else {
...
eld_az_continuity(dgi, vqv, new, zz, bad, prev
		  , fold_shear, scaled_nyqi, qsize, a_fold)
  DGI_PTR dgi;
  struct ve_que_val *vqv;
  short *new, *zz, *prev;
  int bad, fold_shear, scaled_nyqi, qsize, a_fold;
{
    ...
    register int diff;
    short *non_shear_segment(), *aa, *pp, *ppend;
    float ratio;
    ...
    for(aa=new,pp=prev,nv=ns=0; pp < ppend; pp++,aa++) {
	if( *aa != bad && *pp != bad) {
	    nv++;
	    diff = *aa - *pp;
	...
	for(aa=new,pp=prev; pp < ppend; aa++, pp++) {
	    if( *aa != bad && *pp != bad) {
		diff = *aa - *pp;
		if(iabs(diff) > fold_shear) {
 ...
 almost_the_end:
    memcpy(prev, new, nc*sizeof(short));
}
/* c------------------------------------------------------------------------ */

eld_denotch2(dgi, aa, zz, bad, mbc, notch_max, small_shear, notch_shear
	    , scaled_nyqv, qsize)
  DGI_PTR dgi;
  short *aa, *zz;
  int bad, mbc, notch_max, small_shear, notch_shear, scaled_nyqv, qsize;
{
    int i, mark, off, slip=qsize-2;
    short *bb, *cc, *ss;
    short *non_notch_segment(), *non_shear_segment(), *bad_segment();
...
eld_denotch(dgi, ss, zz, bad, mbc, notch_max, small_shear, notch_shear
	    , scaled_nyqv, dir)
  DGI_PTR dgi;
  short *ss, *zz;
  int bad, mbc, notch_max, small_shear, notch_shear, scaled_nyqv, dir;
  /*
...
short *non_shear_segment(aa, zz, shear, num, bad, dir, offs)
  short *aa, *zz;
  int shear, num, bad, dir, *offs;
{
...
short *bad_segment(aa, zz, num, bad, dir)
  short *aa, *zz;
  int num, bad, dir;
{
...
wlee_unfolding(dgi, vqv, aa, zz, bad, qsize, scaled_nyqi, a_fold
	       , scaled_wind, scaled_ac_vel)
  DGI_PTR dgi;
  struct ve_que_val *vqv;
  short *aa, *zz;
  int bad, qsize, scaled_nyqi, a_fold, scaled_wind, scaled_ac_vel;
{
...
eld_unfolding(dgi, vqv, aa, zz, bad, qsize, scaled_nyqi, a_fold, dir)
  DGI_PTR dgi;
  struct ve_que_val *vqv;
  short *aa, *zz;
  int bad, qsize, scaled_nyqi, a_fold, dir;
{
    struct ve_que_val *this;
...
short *non_notch_segment(aa, zz, shear, num, bad, dir, min_val, offs)
  short *aa, *zz;
  int shear, num, bad, dir, min_val, *offs;
{
    /* try to find a run of "num" points where the difference
     * between adjacent points is less than shear
     * this is assummed to be 16-bit data
...
eld_mark_shear(aa, zz, ss, shear, bad)
  short *aa, *zz, *ss;
  int shear, bad;
{
...
cctype16( aa, zz )	/* type as 16 bit integers */
  short *aa, *zz;
{
    int r=0, i, s;
