...
void dd_denotch();
int dd_despeckle();
int dd_fix_vortex_vels();
int dd_gp_threshold();
int dd_legal_pname();
void dd_lidar_db();
int dd_new_param();
void dd_nix_air_motion();
void dd_pct_stats();
int dd_thr_parms();
int dder_lidar_opts();

static int get_kdp();
static struct lidar_parameters * malloc_lidar_parm();
static struct der_pair * malloc_der_pair();
static int setup_der_pairs();
static int setup_der_alts();
static int thr_ldr();
static int zdr_prime();
static void dd_add_bias();
  ...
  DGI_PTR dgi;
{
    ...
    double cosPHI, cosLAM, sinPHI, sinLAM, angdiff(), dd_ac_vel();
    float ac_vel;
...
# endif
	...
	if((a=get_tagged_string("NO_WIND_INFO"))) {
	   ...
	   aa = dont_use_wind_info == USE_PREV_RAY
	     ? "Using first good gate from previous ray."
	       : "Using first good gate in ray.";
...
# endif
    ...
    if( derfs->alt_ders[DUAL_PRF_ALT] && dd_find_field(dgi, "VS") >= 0 &&
  ...
  float threshold;
{
    ...
    scaled_thr = DD_SCALE( threshold, scale, bias );
...
malloc_lidar_parm()
{
    struct lidar_parameters * this =
	(struct lidar_parameters *)malloc(sizeof(struct lidar_parameters));
    memset(this, 0, sizeof(struct lidar_parameters));
    /* default setup is for RDB */
    this->baseline = 0;
    this->counts_per_db = 100.;
    this->end_range = KM_TO_M(3.);
    return( this );
}
/* c------------------------------------------------------------------------ */
...
malloc_der_pair()
{
     ...
     */
    struct der_pair * this =
	(struct der_pair *)malloc(sizeof(struct der_pair));
    memset(this, 0, sizeof(struct der_pair));
    return( this );
}
/* c------------------------------------------------------------------------ */

static int setup_der_pairs( list, der_list )
    char * list;
    struct der_pair ** der_list;
{
    int ii, nt, nn = 0;
    char *aa, *bb, *cc, *dd;
	...
	if( ii + 2 >= nt )
	    { break; }
	if(!ii)
	    { this = malloc_der_pair(); *der_list = this; }
	else
	    { this->next = malloc_der_pair(); this = this->next; }

	strcpy( this->src_name, str_ptrs[ii]);
	strcpy( this->dst_name, str_ptrs[ ii + 2 ]);

	if( !strncmp( this->dst_name, "RDB", 3 )) {
	    derfs->RDB_lps = malloc_lidar_parm();
	}
	if( !strncmp( this->dst_name, "GDB", 3 )) {
...
static int setup_der_alts( list )
    char * list;
{
...
void dd_BB_unfold(dgi, fn, ac_vel, qsize, f_fold_shear, vqv)
  DGI_PTR dgi;
  int fn, qsize;
  float ac_vel, f_fold_shear;
  struct ve_que_val *vqv;
{
    DDS_PTR dds=dgi->dds;
    struct radar_angles *ra=dds->ra;
    struct platform_i *asib=dds->asib;
    ...
    scaled_max_wind = DD_SCALE(f_max_wind, scale, bias);
...
# endif
    ...
    for(sum=0,this=vqv,ii=0; ii < qsize; ii++) { /* load up the que */
	    this->vel = v0;
	    sum += v0;
	    this = this->next;
    }
...
void dd_denotch(dgi, fn, f_notch_max, f_notch_shear)
  DGI_PTR dgi;
  int fn;
  float f_notch_max, f_notch_shear;
{
    DDS_PTR dds=dgi->dds;
    int bad=dds->parm[fn]->bad_data;
    float nyqv=dds->radd->eff_unamb_vel;
...
int dd_despeckle(dgi, fn, a_speckle)
  DGI_PTR dgi;
  int fn, a_speckle;
{
    /*
...
# ifdef obsolete
//	float clean_zdr[MAXNUM_GATES+OFSX+31], clean_rhv[MAXNUM_GATES+OFSX+31] ;
# endif
...
/* Set some initial values */
	...
	count = 0 ;
# ifdef obsolete
//	icount = 1 ;		/* (unused in Fortran code ) */
# endif
...
/* Clear arrays */
	...
	for( i = -5 ; i <= length ; i++ ) {
	    ...
	    x[i+OFSX] = 0 ;
# ifdef obsolete
//	    nad[i+OFSX] = 0 ;		/* preset Pdpadap */
//	    nfl[i+OFSX] = 0 ;		/* preset Pdpadfl */
# endif
	    work_kdp[i+OFSX] = 0 ;	/* preset Kdpadap */
# ifdef obsolete
//	    clean_zdr[i+OFSX] = -100 ;	/* preset bad value for newZdr */
//	    clean_rhv[i+OFSX] = 0 ;	/* preset bad value for newRhv */
# endif
	}
...
# else
   ...
   if((fnx = dd_find_field(dgi, "ZDR")) < 0) {
      return(-1);
   }
   scale = dds->parm[fnx]->parameter_scale;
   rcp_scale = 1./scale;
   ...
   if((fnx = dd_find_field(dgi, "PHI")) < 0) {
      return(-1);
   }
   scale = dds->parm[fnx]->parameter_scale;
   rcp_scale = 1./scale;
   ...
   if((fnx = dd_find_field(dgi, "RHOHV")) < 0) {
      return(-1);
   }
   scale = dds->parm[fnx]->parameter_scale;
   rcp_scale = 1./scale;
...
/*	    for( jj = 0 ; jj <= 2 ; jj++ ) {
		xx[jj+1] = range[i+OFSX-1+jj] ;
		yy[jj+1] = phidp[i+OFSX-1+jj] ;
	    }
	    call LSE(pdpslp,bb,xx,yy,3) !Check pdpslope to avoid ground clutter
*/
...
# ifdef obsolete
//			clean_zdr[i+OFSX-il] = zdr[i+OFSX-il] ;  /* preserve good Zdr & Rhv */
//			clean_rhv[i+OFSX-il] = rhv[i+OFSX-il] ;
# endif
		    }
		    if ( firsttime == 1 ) {
...
# ifdef obsolete
//	    clean_zdr[i+OFSX] = zdr[i+OFSX] ;  /* preserve good Zdr & Rhv */
//	    clean_rhv[i+OFSX] = rhv[i+OFSX] ;
# endif
...
# ifdef obsolete
//		    for ( jj = 0 ; jj < MBAD ; jj++ ) {  /* Inserted to clean the bad Zdr & Rhv. */
//			clean_zdr[i+OFSX-jj] = - 100 ;  /* bad value for Zdr & Rhv */
//			clean_rhv[i+OFSX-jj] = 0 ;
//		    }
# endif
...
/*  PUT KDP,PDPCORRECTED,PDPCORRECTED-FILTERED into PLFILE */
# ifdef obsolete
//	for ( i = -5 ; i <= n+3 ; i++ ) {
//	    nad[i+OFSX] = z[i+OFSZ] ;
//	    nfl[i+OFSX] = y[i+OFSX] ;
...
int dd_single_prf_vortex_vels(dgi, fn, dname)
  DGI_PTR dgi;
  int fn;
  char * dname;
{
    ...
    if((fn_vr = dd_find_field(dgi, "VR")) < 0) {
	return(-1);
    }
	...
	av_nyq = .25 * wvl * prf;
	scaled_av_nyq = DD_SCALE( av_nyq, scale, bias );
...
int dd_fix_vortex_vels(dgi, fn, dname)
  DGI_PTR dgi;
  int fn;
  char * dname;
{
	...
	av_nyqS = .25 * wvl * prfS;
	av_nyqL = .25 * wvl * prfL;
	scaled_av_nyqS = DD_SCALE( av_nyqS, scale, bias );
	scaled_av_nyqL = DD_SCALE( av_nyqL, scale, bias );
...
int dd_gp_threshold(dgi, fn, thr_name, thr_val1, thr_val2, thr_type)
  DGI_PTR dgi;
  int fn, thr_type;
  char *thr_name;
  float thr_val1, thr_val2;
{
...
# endif
	...
	*ss = bad;
	...
	scaled_thr1 = DD_SCALE(thr_val1, dds->parm[fthr]->parameter_scale
	...
	scaled_thr2 = DD_SCALE(thr_val2, dds->parm[fthr]->parameter_scale
			       , dds->parm[fthr]->parameter_bias);
	for(; ss < zz; gg++,ss++,thr++) {
...
int
dd_new_param(dgi, srs_name, dst_name)
  DGI_PTR dgi;
  char *srs_name, *dst_name;
{
    DDS_PTR dds=dgi->dds;
    int ii, fn, fns, fnd, size=0, mark;
    int ns=strlen(srs_name), ndst=strlen(dst_name);
    char *a=NULL, *b=NULL, *cc, *str_terminate(), tmp_name[16], dname[16];
    /*
...
void dd_add_bias(dgi, fn, add_bias)
  DGI_PTR dgi;
  int fn;
  float add_bias;
{
    ...
    scale = dds->parm[fn]->parameter_scale;
    bias = dds->parm[fn]->parameter_bias;
    scaled_bias = DD_SCALE(add_bias, scale, bias);
...
void dd_nix_air_motion(dgi, fn, ac_vel, center_on_zero)
  DGI_PTR dgi;
  int fn, center_on_zero;
  float ac_vel;
{
    DDS_PTR dds=dgi->dds;
    int nc=dgi->clip_gate+1;
...
# define       BEYOND 3
    ...
    int ii, gg, ng, nt, fn, dd_tokens(), scaled_below, scaled_above, mis;
    DDS_PTR dds=dgi->dds;
    static int pct_stats_flag=YES, count=0, which;
    char *a, *b, *get_tagged_string(), string_space[256], *str_ptrs[32];
    ...
    scaled_below = DD_SCALE(below, dds->parm[fn]->parameter_scale
			    , dds->parm[fn]->parameter_bias);
    scaled_above = DD_SCALE(above, dds->parm[fn]->parameter_scale
			    , dds->parm[fn]->parameter_bias);
...
int dder_lidar_opts(derfs, opts, dst_flds, num_dst)
  struct derived_fields *derfs;
  char *opts;
  char *dst_flds[];
{
    int ii, jj, nt, ndx;
