...
double ncswp_time_stamp();

/* c------------------------------------------------------------------------ */

void
nc_dd_conv(interactive_mode)
...
int nc_ini()
{
  ...
  CHAR_PTR aa, bb, cc, ee, get_tagged_string(), sptrs[32];
  char str[256];
  double dtime;
  ...
  if( !difs->num_time_lims ) {
    ns = generic_sweepfiles( aa, slm, "ncswp", "", 0 );
  }
  else {			/* pull out sweepfiles between time limits */
    ns = generic_sweepfiles( aa, slm2, "ncswp", "", 0 );
    slm->num_entries = 0;
...
double ncswp_time_stamp( fname, dts )
     char * fname;
     DD_TIME * dts;
{
...
int nc_access_swpfi( full_path_name )
     char * full_path_name;
{
