...
void sii_update_view_info (guint frame_num)
{
  ...
  vd->orig_values[jj][0] = ff;
  aa = sii_set_string_from_vals (vd->orig_txt[jj], 1, ff, gg, vd->precision[jj]);
...
void sii_view_menu_cb ( GtkWidget *w, gpointer   data )
{
  ...
  case VIEW_HLP_OVERVIEW:
    sii_message (hlp_view_overview);
    break;

  case VIEW_HLP_OPTS:
    sii_message (hlp_view_options);
    break;

  case VIEW_HLP_LINKS:
    sii_message (hlp_view_viewlinks);
    break;

  case VIEW_HLP_CTR:
    sii_message (hlp_view_center);
    break;

  case VIEW_HLP_LMRK:
    sii_message (hlp_view_landmark);
    break;

  case VIEW_HLP_TS:
    sii_message (hlp_view_timeseries);
    break;
		       ...
		       , guint frame_num)
{
   ...
   *menubar = mbar = gtk_menu_bar_new ();
   ...
   vdata->data_widget[widget_id] = menuitem =
     sii_toggle_submenu_item ( "Same as Ref. Frame", submenu, widget_id
			       , sii_view_menu_cb, frame_num, ++radio_num
			       , &radio_group);
