...
static min_rhi_angle_swept=1.5;
static min_ppi_angle_swept=3.;
...
static int etl_next_ray();
static int etl_next_block();
static char *current_file_name;

void etl_dd_conv();
static void etl_ini();
static int etl_next_ray();
static int etl_next_ray_mcaws();
static int etl_next_ray_2micron();
static void etl_positioning();
static void etl_reset();
static int etl_select_ray();
static void etl_nab_data();
static void etl_nab_mom_data();
static void etl_print_stat_line();
static void etl_print_headers();
static void etl_dump_this_ray();
static int etl_inventory();
static void etl_moment_fields();



/* c------------------------------------------------------------------------ */

void etl_dd_conv(interactive_mode) 
...
static void etl_ini()
{
   ...
   if((aa=get_tagged_string("SOURCE_DEV"))) {
      aa = dd_establish_source_dev_names("ETL", aa);
      aa = dd_next_source_dev_name("ETL");
...
etl_next_ray_mcaws()
{
   ...
   char *aa, *bb, *cc = NULL, *ee, *dd_next_source_dev_name(), *dts_print();
   char *tmp_at;
   char *deep6=0;
   char mess[256];
   double d, angle;
   long *lp, xdata_type, data_type_offset, last_data_type;
   ...
   for(;;) {
	 ...
	 if(irq->top->read_state < 0) {
	     ...
	     */
	    if(aa = dd_next_source_dev_name("ETL")) {
	       current_file_name = aa;
...
# ifdef obsolete
	       printf("buf: %8x  at: %8x  data_type: %8x\n"
		     , irq->top->buf, ii*sizeof(long), *(lp + ii));
# endif
	       data_type_offset = ii * sizeof(long);
	       xdata_type = *(lp +ii);
	       break;
	     default:
	       break;
	 ...
	 if(itsa_header) {	/* we've finished reading in a header */
	    ...
	    switch(etui->data_type) {
	     case MC_MOM_HEADER :
	       ...
	       etui->mom_data_ptr = (short *)etui->mom_data;
		...
		default:
	     ...
	     case MC_COV_HEADER :
	       ...
	       etui->cov_data_ptr = (short *)etui->cov_data;
		 ...
		 * mc_cov_ptr->ocs_info.num_lags;
	     ...
	     case MC_RAW_HEADER :
	       ...
	       etui->raw_data_ptr = (short *)etui->raw_data;
	       /*
...
static void etl_print_stat_line(count)
  int count;
{
...
static void etl_dump_this_ray(slm)
