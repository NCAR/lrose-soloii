...
static void init_d_time();

/* c------------------------------------------------------------------------ */

void dd_clear_dts(dts)
  ...
  DD_TIME *dts;
{
    ...
    dts->julian_day = d*SECONDS_TO_DAYS +1.;
    ...
    dts->month = i;
    d -= *(mp+i-1);
    dts->day = d*SECONDS_TO_DAYS;
    d -= (double)((dts->day++)*SECONDS_PER_DAY);
    d -= fudge;
    dts->hour = d*SECONDS_TO_HOURS;
    d -= 3600.*dts->hour;
    dts->minute = d*SECONDS_TO_MINUTES;
    d -= 60.*dts->minute;
    dts->second = d;
    d -= (double)dts->second;
    dts->millisecond = 1000.*d +.5;
    return(dts);
}
/* c----------------------------------------------------------------------- */
