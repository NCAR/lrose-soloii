...
static char *field_names[] = {"DZ", "VE", "SW", "", "", ""};
static char *long_field_names[] = {
    "Reflectivity factor", "Velocity", "Spectral Width", "", "", ""};
static char *field_units[] = {"DBZ", "m/s", "m/s", "", "", ""};
...
void nex_new_parms();
int nex_next_ray();
void nex_update_xtnded_headers();
void nex_fake_xtnded_headers();
void nex_positioning();
void nex_print_dhed();
void nex_print_headers();
void nex_print_nmh();
void nex_print_nvst();
void nex_print_stat_line();
void nex_print_rda();
void nex_reset();
int nex_select_ray();
double nexrad_time_stamp();
  ...
  long *ival;
{
    ...
    int mexp = ((*ival & 0x7f000000) >> 24) - 64;
    int mant =   *ival & 0x00ffffff;
    int precision;
...
void nex_dd_conv(interactive_mode)
...
void nexx_ini()
{
    ...
    char str[256], *line, directory[128], *str_terminate();
    char *a, *aa, *bb, *cc, *get_tagged_string(), *unk="UNKNOWN";
    char *getenv(), string_space[256], *str_ptrs[32];
    double d;
	...
	aa = dd_establish_source_dev_names("NEX", aa);
	aa = dd_next_source_dev_name("NEX");
...
int nex_nab_site_info (struct solo_list_mgmt *slm)
{
   ...
   int latd, latm, lats, lond, lonm, lons;
   static struct nexrad_site_stuff nss2[2];
   char str[256], *line, *str_terminate();
   char *aa, *get_tagged_string(), *unk="UNKNOWN", *getenv();
   const char *bb, *cc;
   double d;
...
void nexx_new_parms()
{
	...
	*r++ = f;
	...
	for(; gg < gri->num_bins; gg++, *(ss++) = gz, gz += ginc) {
	    mark = 0;
	}
	...
	case 2:			/* two bytes */
	    ...
	    *lut++ = EMPTY_FLAG;	/* special flag */
	    for(ii=2; ii < K64; ii++) {
		*lut++ = DD_SCALE(NEX_UNSCALE(ii, rcp_scale, bias)
				  , gri->dd_scale[pn], gri->dd_offset[pn]); 
	    }
	    break;
...
void nex_new_parms()
{
    ...
    *lut++ = nui->options & NEX_RNG_AMB
      ? DD_SCALE(nui->range_ambiguity_flag
		 , gri->dd_scale[fn], gri->dd_offset[fn]) : EMPTY_FLAG;

    for(ii=0; ii < 254; ii++) {
	*lut++ = DD_SCALE(-32. +.5*ii, gri->dd_scale[fn]
			  , gri->dd_offset[fn]); 
    }
    ...
    *lut++ = EMPTY_FLAG;
	...
	for(ii=0; ii < 254; ii++) {
	    *lut++ = DD_SCALE(-127. +ii, gri->dd_scale[fn]
			      , gri->dd_offset[fn]);
	}
    }
    else {
	for(ii=0; ii < 254; ii++) {
	    *lut++ = DD_SCALE(-63.5 +.5*ii, gri->dd_scale[fn]
			      , gri->dd_offset[fn]);
	}
    }
...
char *nex_next_block()
{
	...
	if(iri->top->bytes_left < iri->min_block_size) {
		...
		continue;
	       ...
	       */
	      if(aa = dd_next_source_dev_name("NEX")) {
		current_file_name = aa;
...
# endif
	...
	if( message_type == RDA_STATUS_DATA ) {
	   if(LittleEndian) {
	      nex_crack_rda(at, rda_stat, (int)0);
	   }
	   else {
# ifdef obsolete
...
void
nex_print_dhed(dh, slm)
  struct digital_radar_data_header *dh;
  struct solo_list_mgmt *slm;
{
    int ii;
    char *aa, *bb, str[256];
    DD_TIME dts, *d_unstamp_time();
    double d, nexrad_time_stamp();
    char *dts_print();
...
{
    int ii, jj;
    char *aa, str[128], *str_terminate(), bstr[128];
...
void
nex_print_nmh(mh, slm)
  struct nexrad_message_header *mh;
  struct solo_list_mgmt *slm;
{
    int ii;
    char *aa, str[128], *str_terminate();
...
void
nex_print_rda( rda, slm )
  struct rda_status_info *rda;
  struct solo_list_mgmt *slm;
{
   int ii;
   char *aa, str[128], *str_terminate();
...
void
nex_print_nvst(vst, slm)
  struct nexrad_vol_scan_title *vst;
  struct solo_list_mgmt *slm;
{
    int ii;
    char *aa, str[128], *str_terminate(), bstr[128];
...
void nex_print_stat_line(count)
  int count;
{
  ...
  DD_TIME *dts;
{
    ...
    dts->year = 1970;
    dts->month = dts->day = 0;
    dts->day_seconds = ms*.001 +(jday-1)*SECONDS_PER_DAY;
    d = d_time_stamp(dts);
    return(d);
}
/* c------------------------------------------------------------------------ */
