...
void cimm_dd_conv();
void cimm_dump_this_ray();
void cimm_ini();
int cimm_inventory();
void cimm_nab_data();
int cimm_next_ray();
void cimm_positioning();
void cimm_print_header();
void cimm_print_stat_line();
void cimm_reset();
int cimm_select_ray();
int cimm_isa_new_sweep();
...
void cimm_dd_conv(interactive_mode)
...
# endif
	...
	*( pdbm + gg ) = DD_SCALE( dbm_cal[ ic ], scale, bias );
	d_dbz = dbz_cal[ ic ];
	*( pdbz + gg ) = DD_SCALE( d_dbz + rng_norm[gg], scale, bias );
	d_rcc = *( cc + gg ) * .01;
# ifdef obsolete
	d_snr = snr_cal[ ic ];
	StoN = WATTZ( d_snr );
...
# endif
	...
	*( prho + gg ) = DD_SCALE( d_rho, scale, bias );

	*( pphi + gg ) = DD_SCALE( *( qdp + gg ), scale, bias );
	*( pzdr + gg ) = DD_SCALE( *( zdr + gg ) * .05, scale, bias );
	*( psw + gg ) = DD_SCALE( *( sw + gg ) * .25, scale, bias );
	*( pve + gg ) = DD_SCALE( *( vel + gg ), scale, bias );
# ifdef INCLUDE_RCC
...
# endif    
    ...
    cmui->fxd_ang_sum += gri->scan_mode == PPI ? gri->elevation
	: gri->azimuth;
    ++cmui->sweep_ray_num;
