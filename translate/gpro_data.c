...
double gpro_decode();
char *gpro_next_rec();
void gpro_position_err();
struct ads_raw_data_que *gpro_sync();
struct ads_raw_data_que *gpro_initialize();
...
# ifdef NETCDF
    ...
    int ii, mark, ndx;
    DD_TIME dtx, *d_unstamp_time();
    char *aa, *dts_print(), mess[256];
  ...
  int options;
{
    ...
    residue = 1. - (dts->time_stamp - (long)dts->time_stamp);
...
ac_nc_init()
{
    ...
    long ll;
    float f;
    char *aa, *bb, *cc, *tag, *get_tagged_string(), name[256];
    char *getenv(), var_name[16], string_space[256], *str_ptrs[66];
	 ...
	 */
	ndx = ndx_time_offset;
	ac_var_tags[ndx] = "time_offset";
	strcpy(ac_var_names[ndx], ac_var_tags[ndx]);

	ndx = ndx_HGME;
	ac_var_tags[ndx] = "HGME";
	strcpy(ac_var_names[ndx], ac_var_tags[ndx]);

	ndx = ndx_PITCH;
	ac_var_tags[ndx] = "PITCH";
	strcpy(ac_var_names[ndx], ac_var_tags[ndx]);

	ndx = ndx_ROLL;
	ac_var_tags[ndx] = "ROLL";
	strcpy(ac_var_names[ndx], ac_var_tags[ndx]);

	ndx = ndx_THDG;
	ac_var_tags[ndx] = "THDG";
	strcpy(ac_var_names[ndx], ac_var_tags[ndx]);

	ndx = ndx_VEWC;
	ac_var_tags[ndx] = "VEWC";
	strcpy(ac_var_names[ndx], ac_var_tags[ndx]);

	ndx = ndx_VNSC;
	ac_var_tags[ndx] = "VNSC";
	strcpy(ac_var_names[ndx], ac_var_tags[ndx]);

	ndx = ndx_VSPD;
	ac_var_tags[ndx] = "VSPD";
	strcpy(ac_var_names[ndx], ac_var_tags[ndx]);
...
# endif
	ndx = ndx_LATC;
	ac_var_tags[ndx] = "LATC";
	strcpy(ac_var_names[ndx], ac_var_tags[ndx]);

	ndx = ndx_LONC;
	ac_var_tags[ndx] = "LONC";
	strcpy(ac_var_names[ndx], ac_var_tags[ndx]);

	ndx = ndx_PALT;
	ac_var_tags[ndx] = "PALT";
	strcpy(ac_var_names[ndx], ac_var_tags[ndx]);
...
void eld_gpro_fix_asib(dts, asib, options)
  DD_TIME *dts;
  struct platform_i *asib;
  int options;
{
    static int count=0, trip_count=500;
    char *get_tagged_string(), *gpro_next_rec();
    struct ads_raw_data_que *rdq;
    struct ads_raw_data_que *gpro_sync();
    float delta;
    int i, status, mark;
    double palt_corr, d, d_time_stamp();
    ...
    if(options AND TOGACOARE_FIXES) {
...
void gpro_assemble_vals(rdq, adsd, pct)
  struct ads_raw_data_que *rdq;
  float pct;
  struct ads_data *adsd;
{
    int hr, min, sec;
    double d, track, gpro_decode(), d_time_stamp();
    /*
...
# endif
    ...
    adsd->ptime = gpro_decode(ltvi_list[GPRO_ID_PTIME]
			      , pct, rdq->at[GPRO_ID_PTIME]);
    hr = gpro_decode(ltvi_list[GPRO_ID_HR]
			      , pct, rdq->at[GPRO_ID_HR]);
    min = gpro_decode(ltvi_list[GPRO_ID_MIN]
			      , pct, rdq->at[GPRO_ID_MIN]);
    sec = gpro_decode(ltvi_list[GPRO_ID_SEC]
...
# endif
	...
	for(i=0,ltvi=ltvi_top; i < ltvi_count; i++,ltvi=ltvi->next) {
	    rdq->at[ltvi->id_num] = (long *)(rdq->raw_data_buf +ltvi->offset);
	}
    }
}
/* c------------------------------------------------------------------------ */
...
void gp_header( buf )
  char *buf;
{
    char *a, *s=buf, *gp_arg_ptr();
    char tmp[88];
    int i, m, n, gp_get_line(), len_arg(), count=0;
    float gp_time();
    /*
...
int gp_get_line( s )			/* CRAY version */
  char *s;
{
...
double
gpro_decode(this, pct_samp, at)
  struct letvar_info *this;
  float pct_samp;
  long *at;
{
    int sample=this->sample*pct_samp;
    double d;
  ...
  int *status;
{
    ...
    double d, gpro_decode(), d_time_stamp();
...
struct ads_raw_data_que *
gpro_initialize(adsd_at)
  struct ads_data **adsd_at;
{
    char *get_tagged_string();
