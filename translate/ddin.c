...
void dgi_interest(dgi, verbosity, preamble, postamble)
  struct dd_general_info *dgi;
  int verbosity;
...
int ddin_ini(interactive)
  int interactive;
{
    int ii, kk, n, mark;
    char *aa, *get_tagged_string(), *strstr();
	...
	aa = dd_establish_source_dev_names("DDI", aa);
	aa = dd_next_source_dev_name("DDI");
...
void ddin_init_sweep(dgi, current_time, new_vol)
  DGI_PTR dgi;
  time_t current_time;
  int new_vol;
{
...
int ddin_next_ray()
{
    ...
    struct dd_comm_info *dd_comm, *dd_next_comment();
	...
	while(irq->top->bytes_left <= sizeof(struct generic_descriptor)) {
		 ...
		 */
		if(aa = dd_next_source_dev_name("DDI")) {
		    dui->current_file_name = aa;
	...
	if(LittleEndian) {
	   ...
	   swack4(ddin_next_block+4, &gd->sizeof_struct);
	}
	else {
	...
	if(gd->sizeof_struct < sizeof(struct generic_descriptor)) {
	   ...
	   if( !dgi->dds->uniform_cell_count[0] ) {
	    ...
	    if(LittleEndian) {
	       ddin_crack_qdat(ddin_next_block, dds->qdat[pn], (int)0);
	    }
	    else {
	     ...
	     */
	    if(LittleEndian) {
	       ddin_crack_ryib(ddin_next_block, dds->ryib, (int)0);
	    }
	    else {
	    ...
	    if(LittleEndian) {
	       ddin_crack_asib(ddin_next_block, dds->asib, (int)0);
	    }
	    else {
	    ...
	    if(LittleEndian) {
	       ddin_crack_swib(ddin_next_block, &diri->swib, (int)0);
	    }
	    else {
	    ...
	    dds->field_present[pn] = YES;
	    if(LittleEndian) {
	       ddin_crack_parm(ddin_next_block, dds->parm[pn], (int)0);
	    ...
	    dgi->parm_type[pn] = !(jj == DBZ_ID_NUM || jj == DZ_ID_NUM)
		  ? VELOCITY : OTHER;
	    str_terminate(diri->parm_names[pn]
	   ...
	   gotta_celv = NO;
	    ...
	    if(LittleEndian) {
	       ddin_crack_radd(ddin_next_block, dds->radd, (int)0);
	    }
	    else {
	   ...
	   gotta_celv = YES;
	    if(LittleEndian) {
	       ddin_crack_celv(ddin_next_block, dds->celv, (int)0);

	       swack_long(ddin_next_block+12, &dds->celv->dist_cells[0]
	   ...
	   gotta_cfac = YES;
	    ...
	    if(!ignore_cfacs) {
	       if(LittleEndian) {
		  ddin_crack_cfac(ddin_next_block, dds->cfac, (int)0);
	       }
	       else {
	   ...
	   if(gotta_celv) {	/* in case the celv is before the cfac */
	    ...
	    ryib_flag = NO;
	    if(LittleEndian) {
	       ddin_crack_vold(ddin_next_block, vold, (int)0);
	    }
	    else {
  ...
  struct input_read_info *iri;
{
    ...
    long mm=iri->sizeof_file, oo=iri->top->offset;
    char *c=iri->top->buf, *strstr(), name[8];
...
int ddin_select_ray(dgi)
  DGI_PTR dgi;
{
    int ii, jj, n, mark, rn=dui->radar_num, ok=YES;
    float az, el;
    double dd_azimuth_angle(), dd_elevation_angle(), d;
    char *a;
    static struct dor_inidividual_radar_info *diri;
    diri = dui->diri[rn];
...
void ddin_stuff_ray(dgi, current_time)
  DGI_PTR dgi;
  time_t current_time;
{
    /*
