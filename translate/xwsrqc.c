...
int wsrqc_correct();
int wsrqc_init();
struct wsrqc_stuff_ptrs *wsr_malloc();
int nab_wsrqc_stuff();
int wsrqc_power_cor();
...
main()
{
    struct generic_radar_info *gri, *return_gri_ptr();

    long ival=0x40000004;
    double d, cvt_hp1000fp(), d_time_stamp();
    int ii, jj, kk, nn;
  ...
  char *name;
{
	...
	if(gri->time >= wsp3->next->at->start_time) {
	     ...
	     */
	    if((mm = nab_wsrqc_stuff(stream, buf, wsp3->next, "scan", 1)) < 2) {
  ...
  struct generic_radar_info *gri;
{
     ...
     */
    char *a, *get_tagged_string(), *strstr();
    int ii, jj, kk, mm, nn, mark;
    struct wsrqc_stuff_ptrs *wsp, *last_wsp, *wsr_malloc();
    long fpos, flen, finc;
     ...
     */
    if(!(mm = nab_wsrqc_stuff(stream, buf, wsp, "scan", 1))) {
    ...
    for(ii=0;; ii++) {
	wsp = wsr_malloc();
	mm = nab_wsrqc_stuff(stream, buf, wsp, "scan", 1);
	if(!ii) {
	    top_scope2 = wsp;
	}
	else {
	 ...
	 */
	last_line = NULL;
	mm = nab_wsrqc_stuff(stream, buf, wsp->next, "scan", 2);
	/*
    ...
    current_offset = ftell(stream);
    last_line = NULL;
    mm = nab_wsrqc_stuff(stream, buf, wsp, "scan", 2);
...
int nab_wsrqc_stuff(stream, buf, wsp, quit_at, num_occurs)
  FILE *stream;
  char *buf, *quit_at;
  struct wsrqc_stuff_ptrs *wsp;
  int num_occurs;
{
	...
	current_offset = ftell(stream); /* save the position
					 * of this line */
	last_line = c;
	if(!(a = fgetsx(last_line, (int)88, stream))) {
	    break;
	}
	lc++;
	c += strlen(c);
