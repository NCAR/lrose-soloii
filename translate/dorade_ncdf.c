...
struct rnc_info * return_rnc_info( char * name )
{
  ...
  static char * long_names [] =
  {
    ...
    , "Std. Dev. of PhiDp"
    , "Std. Dev. of Raw Doppler Velocities"
    , "Std. Dev. of Zdr"
    , "Hail Detection Ratio"
    , "Temperature"
  };
  ...
  ri->field_type = field_types[match];
...
struct input_read_info *return_pir_irq();
...
void produce_ncdf();
static void ncp_stuff_ray();
static void ncdf_finish();
static void ncdf_init_sweep();

static int dd_setFxdR1GsEtc();
static int dd_setRcvrXmtrStuff();
static int dd_setLatLonAlt();
static int dd_setACparams();
static int dd_setAzEl();
void return_piraq_radar_consts();
...
static char * horiz_and_vert_fields_list()
{
  static char list[256];
...
# endif	
	...
	if( aa = get_tagged_string("NC_DIR") ) {
	  ...
	  slash_path( ncs->dir_name, dgi->directory_name );
	}
...
static int dd_setFxdR1GsEtc( ncp, num_dims, dims )
struct ncdf_production *ncp;
int num_dims, *dims;
{
...
static int dd_setRcvrXmtrStuff( ncp, num_dims, dims )
struct ncdf_production *ncp;
int num_dims, *dims;
{
  char str[256], *aa, *bb, *cc;
  ...
  ii = nc_put_att_text( ncid, ncp->id_rcvrg
			, "long_name", strlen( str ) +1, str );
  aa = "Most entries are 2 dimension arrays one for each polarity";
  strcpy( str, aa );
  ii = nc_put_att_text( ncid, ncp->id_rcvrg
...
static int dd_setLatLonAlt( ncp, num_dims, dims )
struct ncdf_production *ncp;
int num_dims, *dims;
...
static int dd_setAzEl( ncp, num_dims, dims )
struct ncdf_production *ncp;
int num_dims, *dims;
{
  char str[256], *aa, *bb, *cc;
...
static int dd_setACparams( dgi, ncp, num_dims, dims )
     struct dd_general_info *dgi;
     struct ncdf_production *ncp;
     int num_dims, *dims;
{
  ...
  if( dgi->dds->radd->scan_mode == AIR ) {
				/*
				  // tail
				 */
    aa = "Degrees clockwise from zenith ";
  }
  else {
				/*
    // nose or lower fuselage
				 */
    aa = "Degrees clockwise from forward";
  }
  strcpy( str, aa );
  ii = nc_put_att_text( ncid, ncp->id_rotang
			, "Comment", strlen( str ) +1, str );
  ...
  if( dgi->dds->radd->scan_mode == AIR ) {
    ...
    // tail
				 */
    aa = "Degrees from starboard perpendicular towards forward";
  }
  else {
				/*
    // nose or lower fuselage
				 */
    aa = "Clockwise angle from platform horizontal towards the ray ";
  }
  strcpy( str, aa );
  ii = nc_put_att_text( ncid, ncp->id_tilt
			, "Comment", strlen( str ) +1, str );
