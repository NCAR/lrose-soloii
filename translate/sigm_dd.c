...
void sigmet_dd_conv();
void sigmet_ini();
void sigmet_nab_data();
void sigmet_nab_info();
int sigmet_next_ray();
int sigmet_select_ray();
void sigmet_print_stat_line();
int ingest_ingest_headers();
void grab_summary_header();
int sig_read();
...
void sigmet_nab_info()
{
    ...
    double sigmet_time_stamp(), fabs(), fmod(), sin(), cos();
    float a1, a2;
    double d, angdiff(), fmod();
    DD_TIME *dts=gri->dts;
    ...
    if( gri->scan_mode == PPI || gri->scan_mode == SUR ) {
	...
	if((d = .001*X4(eh->itimems)) > (1.1*SECONDS_PER_DAY)) {
	    mark = 0;
	}
	else if(d > 90. ) {
	    mark = 0;
...
# endif
	...
	if(new_run = iter >= rlcount)
	      for(usx=us; X2(*usx++) == SIGMET_END_OF_RAY; eors++);
...
void sigmet_print_stat_line(count)
  int count;
{
    time_t i;
    double d;
...
int ingest_ingest_headers(tbuf)
  char *tbuf;
{
...
# endif
	...
	case DB_VEL_P:
	    ...
	    for(i=1; i < 256; i++ ) {
		*ss++ = S100((i-128)*.007874*trgr*gri->nyquist_vel);
	    }
# ifdef obsolete
...
# endif
	...
	case DB_WIDTH_P:
	    ...
	    for(i=1; i < 256; i++ ) {
		*ss++ = S100(i*.00390625*trgr*gri->nyquist_vel);
	    }
# ifdef obsolete
...
long sig_swap4( ov )		/* swap integer*4 */
  fourb *ov;
{
    union {
	long newval;
