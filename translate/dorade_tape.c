...
static struct dd_sweep_list_entries *dd_sweep_listit();
static void dd_tape_dump();


/* c------------------------------------------------------------------------ */

static struct dd_sweep_list_entries
*dd_sweep_listit (dgi, swp_que, this, sweep_count)
  DGI_PTR dgi;
  struct prev_swps *swp_que;
  struct dd_sweep_list_entries *this;
  int *sweep_count;
{
     ...
     */
    /* c...mark */
    int mark, ii, rn, dd_min_rays_per_sweep();
    char *dts_print();
    DD_TIME *d_unstamp_time();

    if(swp_que->num_rays >= dd_min_rays_per_sweep() && !swp_que->listed
       && !dgi->ignore_this_sweep) {
	this->volume_time_stamp = swp_que->volume_time_stamp;
	this->sweep_time_stamp = swp_que->sweep_time_stamp;
	this->start_time = swp_que->start_time;
	this->end_time = swp_que->end_time;
	this->segment_num = swp_que->segment_num;
	this->source_sweep_num = swp_que->source_sweep_num;
	this->radar_num = rn = dgi->radar_num;
	this->new_vol = swp_que->new_vol;
	this->num_rays = swp_que->num_rays;
	this->num_parms = swp_que->num_parms;
	this->ignore_this_sweep = NO;
	this->next_segment = NULL;
	(*sweep_count)++;
	swp_que->listed = YES;
	strcpy(this->file_name, swp_que->file_name);
# ifdef obsolete
	dts.time_stamp = this->start_time;
...
# endif
	if(nv_track->start_time < 0 && rn != nv_track->LF_radar_num) {
	    nv_track->start_time = this->start_time;
	}
	...
	this->nv_track_last = nv_track->swle_last[rn];
	nv_track->swle_last[rn] = this;
	nv_track->sweep_count[rn]++;
	nv_track->end_time = this->end_time;
  ...
  int flush;
{
    ...
    static struct dd_sweep_list_entries *top=NULL
	  , *this=NULL, *next=NULL, *last, **swp_list=NULL;

    struct dd_general_info *dgii, *dd_get_structs();
    struct dd_sweep_list_entries *dd_sweep_listit();
    struct dd_input_filters *dd_return_difs_ptr();
    int i=0, n, rn, dump_em=NO, mark, radar_num = -1, nt, dd_tokens();
    double d, et, atof();
    char *a, *get_tagged_string(), *dfn, str[256], *dd_radar_name();
	 ...
	 */
	for(i=0; i < 64; i++) {
	    this = (struct dd_sweep_list_entries *)
		  malloc(sizeof(struct dd_sweep_list_entries));
	    memset((char *)this, 0, sizeof(struct dd_sweep_list_entries));
	    if(!i) {
		next = this;
		if(!max_sweep_count) 
		      top = this;
	...
	for(rn=0; rn < dd_num_radars(); rn++) {
		...
		if(dgii->dds->radd->radar_type == AIR_LF)
		      continue;
...
dd_tape_dump( top, sweep_count, out_fid, remove_them, io_type
	     , vol_num, swp_list)
  struct dd_sweep_list_entries *top, **swp_list;
  int sweep_count, out_fid, remove_them, io_type, vol_num;
{
    ...
    DGI_PTR dgi, dd_get_structs();
    DD_TIME *d_unstamp_time();
    struct dd_sweep_list_entries *this, *that, *last, *dsle[MAX_SENSORS];
    ...
    struct ray_i *ryib;
    struct comment_d *comm, *dd_return_comment();

    double d, d_time_stamp(), keep_time, dorade_time_stamp();
    ...
    long cdl;
    char *a, *sb, *tb, *dd_find_desc(), *dts_print(), *first_id;
    char str[256], *strrchr(), *dd_radar_name(), *str_terminate();
    DD_TIME ddts;
     ...
     */
    for(this=top,sn=0; sn < sweep_count; sn++,this=this->next) {
	/* assemble the segments for this sweep
	 */
	if(!this->ignore_this_sweep) {
	    real_sweep_count++;
	    *swp_list_ptr++ = this;
	    last = this;
	    for(that=this->next,snn=sn+1; snn < sweep_count; snn++) {
		if(!that->ignore_this_sweep &&
		   this->radar_num == that->radar_num &&
		   this->source_sweep_num == that->source_sweep_num) {
		    /*
		     * this is another segment of the same sweep
		     */
		    printf("Segmented sweep %d %3d %.3f "
			   , this->radar_num
			   , this->source_sweep_num
			   , last->start_time);
		    ...
		    this->num_rays += that->num_rays; /* accululate num_rays */
		}
		that = that->next;
	    }
	}
...
# endif
	for(j=0; j < radar_count; j++) {
	    if(dsle[j]->radar_num == this->radar_num)
		  break;
	}
	if(j == radar_count) {	/* add this radar */
	    /* make sure we can rally access this sweep */
	    dgi = dd_get_structs(this->radar_num);
	    strcpy(str, dgi->directory_name);
	    strcat(str, this->file_name);
	    if((swp_fid = open(str,0)) < 1 ) {
		printf("\nUnable to open %s status: %d ... Ignoring it!\n"
		       , str, swp_fid);
		this->ignore_this_sweep = YES;
	    }
	...
	strcpy(str, dgi->directory_name);
	strcat(str, this->file_name);
	printf("Vol Dump %s %d ", str, this->sweep_time_stamp);
	...
	if(!(a = dd_find_desc(sbuf, sbuf+slen, "RYIB"))) {
	...
	if(!rn) {
	     ...
	     */
	    if(!(a = dd_find_desc(sbuf, ryib, "VOLD"))) {
		mark = 0;
	    }
	    gd = (struct generic_descriptor *)a;
	    if(LittleEndian){
	    ...
	    if((a = dd_find_desc(sbuf, ryib, "COMM"))) {
		...
		*/
		...
		if(LittleEndian){
		   ...
		   swack4(&cdl, &comm->comment_des_length);
		}
		vsize += gd->sizeof_struct;
	    }
	}
	 ...
	 */
	if(!(a = dd_find_desc(sbuf, ryib, "RADD"))) {
	   mark = 0;
	}
	gd = (struct generic_descriptor *)a;
	 ...
	 */
	if(!(a = dd_find_desc(sbuf, ryib, "PARM"))) {
	    /* complain */
	   mark = 0;
	}
	for(i=0; i < npd; i++) {
	...
	if(!(a = dd_find_desc(sbuf, ryib, "CELV"))) {
	   mark = 0;
	}
	gd = (struct generic_descriptor *)a;
	if(LittleEndian){
	   ...
	   swack_long(&celv->dist_cells[0], vtbuf+vsize +12
	...
	if(!(a = dd_find_desc(sbuf, ryib, "CFAC"))) {
	   mark = 0;
	}
	gd = (struct generic_descriptor *)a;
	if(LittleEndian){
	...
	for(;;) {
	    ...
	    printf("Copy s:%2d %s %.3f %.2f MB %d"
		   , sn, strrchr(str, '/'), this->start_time
		   , f_bytes*1.e-6, this->source_sweep_num);
	    if((swp_fid = open(str,0)) < 1 ) {
		printf("\nUnable to open %s status: %d\n", str, swp_fid);
		break;
	    }
...
"Write err fid: %d count: %d status: %d %d\n"
	    ...
	    close(swp_fid);
	    if(remove_them) {
		if(sweep_keep_int > 0 && sweep_keep_stamp[this->radar_num] +
		   sweep_keep_int <= this->start_time) {
		    sweep_keep_stamp[this->radar_num] = this->start_time;
		}
		else
		      n = unlink(str);
	    }
	    if(this->next_segment)
		  this = this->next_segment;
	    else
		  break;
	}
    }
