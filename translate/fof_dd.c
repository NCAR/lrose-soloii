...
static char *Months[] = { "000", "JAN", "FEB", "MAR", "APR", "MAY", "JUN",
			     "JUL", "AUG", "SEP", "OCT", "NOV", "DEC", "013" };
...
static char *final_fields="DM VR";
...
void fof_dd_conv();
void fof_absorb_catalog_header();
struct source_field_info *fof_check_srs_fld();
void fof_catalog_header();
FILE *fof_catalog_header_stream();
double fof_crack_vms_date();
struct data_field_info *fof_data_field_spair();
void fof_dump_this_ray();
struct data_field_info *fof_duplicate_field();
void fof_era();
struct data_field_info *fof_establish_data_field();
struct source_field_info *fof_find_srs_fld();

void fof_gen_counts_field();
void fof_gen_dbz_field();
void fof_gen_field();
void fof_gen_ldr_field();
void fof_gen_noiseless_zdr();
void fof_gen_unfolded_linear();
void fof_gen_quotient_field();
void fof_gen_snr();
void fof_gen_sw();
void fof_gen_thrd_field();
void fof_gen_unfolded_xvert();
char *fof_get_field_list_name();
void fof_header_list();
void fof_ini();
struct data_field_info *fof_insert_data_field();
int fof_inventory();
int fof_isa_new_sweep();
int fof_isa_new_vol();
void fof_link_cat_info();
void fof_load_field_list();
void fof_luts();
void fof_malloc_vals();
/*
 * new sweep and new volume delineation algorithms
 */
void fof_MHR_or_cape();
void fof_mist_cp3();
void fof_nab_data();
struct fof_sweep_que *fof_new_sweep();
void fof_new_vol();
int fof_next_ray();
void fof_positioning();
struct source_field_info *fof_possible_field();
void fof_print_fieldlist();
void fof_print_headers();
void fof_print_hsk();
void fof_print_stat_line();
int fof_process_cal_info();
void fof_process_field_info();
void fof_process_system_info();
struct data_field_info *fof_push_data_field();
struct data_field_info *fof_que_data_field();
struct fof_ray_que *fof_que_ray_info();
struct source_field_info *fof_raw_counts_field();
void fof_rayq_info();
struct fof_ray_que *fof_ray_que_spair();
void fof_redo_srsfs();
struct data_field_info *fof_remove_data_field();
void fof_reset();
void fof_rng_corr();
int fof_select_ray();
void fof_release_data_fields();
void fof_setup_field();
void fof_stack_ray_que_spairs();
void fof_stack_sweep_que_spairs();
struct fof_sweep_que *fof_sweep_que_spair();
double fof_time();
int zatt_arg_ptrs();
int xnext_att();
int znext_att();
...
void fof_dd_conv(interactive_mode)
  int interactive_mode;
{
    /* main loop for converting fof data
     */
    int i, n, mark;
    static count=0, nok_count;
    struct data_field_info dataf;  
...
void fof_absorb_catalog_header(fp)
  FILE *fp;
{
...
struct source_field_info *
fof_check_srs_fld(field_name, derfx)
  char *field_name;
  struct field_derivation_info **derfx;
{
    /* 
     * 
     */
...
void fof_catalog_header()
{
    char *a, hfile[64], str[256], tm[24], *dts_print();
    char *dir="/data/dmg/catalog/header/", *hlj="headerlist.job";
    double ztime;
    RSF_HOUSE *hsk = (RSF_HOUSE *)fui->ray_que->at;
    FILE *fp, *fof_catalog_header_stream();
    DD_TIME dts, *d_unstamp_time();
    double d, d_time_stamp();
    DGI_PTR dd_get_structs();
...
fof_catalog_header_stream()
{
    char *a, *b, *c, hfile[64], str[256], tm[24], *dts_print(), *getenv();
    char *fcat_hdrs="/rdss/solo/catalog/header/";
    char *hldj="headerlist.job";
    FILE *fp;
    DD_TIME dts, *d_unstamp_time();
    double d, d_time_stamp();
...
double
fof_crack_vms_date(sptrs, nargs, dts)
  char **sptrs;
  int nargs;
  DD_TIME *dts;
{
...
fof_data_field_spair()
{
     ...
     */
    struct data_field_info *this=data_field_spairs;

    if(!this) {
	this = data_field_spairs = SMALLOC(struct data_field_info);
	if(!this) {
	    printf("Could not allocate another data_field_spair\n");
	    exit(1);
	}
	memset(this, 0, sizeof(struct data_field_info));
    }
    data_field_spairs = data_field_spairs->next;
    this->missing = EMPTY_FLAG;
    this->dd_scale = 100.;
    this->raw_counts = NO;
    return(this);
}
...
struct data_field_info *
fof_duplicate_field(field_name)
  char *field_name;
{
...
struct data_field_info *
fof_establish_data_field(field_name)
  char *field_name;
{
    int after=NO;
    struct data_field_info *dataf, *fof_data_field_spair();
    struct source_field_info *fof_possible_field();
...
fof_get_field_list_name()
{
    char *field_list="/rdss/solo/catalog/fieldlist.job";
    char *a, *getenv();
...
void fof_header_list(fp, name, ztime, headername, experiment, instrument
		, start, stop)
  FILE *fp;
  char *name, *headername, *experiment, *instrument;
  double ztime, *start, *stop;
{
...
void fof_ini()
{
    ...
    struct fof_sweep_que *sq, *fof_sweep_que_spair();
    struct field_derivation_info *derf;
    struct source_field_info *srsf;
    struct final_field_info *finalf;
    struct fof_ray_que *rq, *fof_ray_que_spair(), *fof_que_ray_info();
    char str[256], *fof_get_field_list_name();
    char string_space[256], *str_ptrs[32], *ffn;
    char *a, *get_tagged_string(), *unk="UNKNOWN", *field_name;
    float f;
    double d, atof();
    FILE *fp;
...
struct data_field_info *
fof_insert_data_field(this, that, before)
  struct data_field_info *this, *that;
  int before;
{
  ...
  struct input_read_info *iri;
{
    ...
    char str[256];
    double d;
    struct io_packet *dp, *dd_return_current_packet();
  ...
  struct source_field_info *srsf;
{
    ...
    DD_TIME dts;
    double d, dtime, d_time_stamp(), xp, mantissa;
...
void
fof_MHR_or_cape(its_changed, rq)
  int its_changed;
  struct fof_ray_que *rq;
{
    double d;
  ...
  struct fof_ray_que *rq;
{
    ...
    struct fof_sweep_que *sq, *fsq, *fof_sweep_que_spair();
    struct final_field_info *finalf;
    struct data_field_info *dataf, *fof_data_field_spair();
    struct data_field_info *fof_duplicate_field();
    struct data_field_info *fof_establish_data_field();
    struct source_field_info *fof_possible_field();
    struct source_field_info *fof_raw_counts_field();
    struct source_field_info *srsf, *fof_find_srs_fld(), *fof_check_srs_fld();
    struct field_derivation_info *derf;
    struct fof_system_attributes *fsi;
    RSF_HOUSE *hsk;
    unsigned short *us;
    char *a, *b, *c, *str_terminate(), str[88];
    float r, gs;
    double d;
...
int fof_next_ray()
{
    ...
    struct fof_ray_que *rq, *fof_que_ray_info(), *fof_ray_que_spair();
    struct fof_sweep_que *fsq, *fof_new_sweep();
...
void fof_positioning()
{
    ...
    float f;
    double d, tol, dd_relative_time(), dtarget, d_time_stamp();
...
struct source_field_info *
fof_possible_field(field_name, derf)
  char *field_name;
  struct field_derivation_info **derf;
{
    struct source_field_info *srsf, *fof_check_srs_fld();
    struct source_field_info *fof_find_srs_fld();
  ...
  struct solo_list_mgmt *slm;
{
	...
	if(MATCHN(att, "source:", 4) || MATCHN(att, "dfield:", 4))
	      solo_add_list_entry(slm, att, strlen(att));
    }
    fclose(fp);
  ...
  struct solo_list_mgmt *slm;
{
    ...
    solo_reset_list_entries(slm);
    solo_add_list_entry(slm, " ", 1);
    solo_add_list_entry(slm, header_info, strlen(header_info));
    solo_add_list_entry(slm, " ", 1);
...
int fof_process_cal_info(fp)
  FILE *fp;
{
    int ii, jj, kk, limit;
    char *a, att[1024], string_space[1024], *str_ptrs[32];
    char *get_tagged_string();
    double d, atof(), A, B, noise;
    struct calibration_info *zcal;
...
void fof_process_field_info(fp)
  FILE *fp;
{
    int ii, jj, kk;
    char *a, att[1024], string_space[1024], *str_ptrs[32];
...
void fof_process_system_info(fp)
  FILE *fp;
{
	...
	xradd = dd_malloc_radd(dgi, "XBAND   ");
...
{
    int before=YES;
    struct data_field_info *this, *fof_insert_data_field();

    return(this = fof_insert_data_field(dataf, fui->dataf, before));
}
/* c------------------------------------------------------------------------ */
  ...
  struct data_field_info *dataf;
{
    ...
    return(this = fof_insert_data_field
	   (dataf, (fui->dataf ? fui->dataf->last : fui->dataf), after));
}
/* c------------------------------------------------------------------------ */

struct fof_ray_que *
fof_que_ray_info(top, this)
  struct fof_ray_que **top, *this;
{
...
struct source_field_info *
fof_raw_counts_field(field_name)
  char *field_name;
{
...
fof_ray_que_spair()
{
    struct fof_ray_que *this=ray_que_spairs;

    if(!this) {
	this = ray_que_spairs = SMALLOC(struct fof_ray_que);
	if(!this) {
	...
	ray_que_count++;
	memset(this, 0, sizeof(struct fof_ray_que));
    }
    ray_que_spairs = ray_que_spairs->next;
    this->next = this->last = this->prev = NULL;

    return(this);
}
/* c------------------------------------------------------------------------ */

void fof_redo_srsfs(rq)
  struct fof_ray_que *rq;
{
    int ii, jj, kk, nn, mark, ok, before=YES, after=NO;
    struct final_field_info *finalf;
    ...
    *gri->source_field_mnemonics = '\0';
    ...
    hsk = (RSF_HOUSE *)rq->at;
    fui->ref_srsf = srsf = fof_check_srs_fld("DZ", &derf);
...
struct data_field_info *
fof_remove_data_field(this)
  struct data_field_info *this;
{
    struct data_field_info *top=fui->dataf;
...
void fof_release_data_fields(this)
  struct data_field_info *this;
{
    if(!this)
	  return;
    this->last->next = data_field_spairs;
    data_field_spairs = this;
}
/* c------------------------------------------------------------------------ */

void fof_setup_field(dataf)
  struct data_field_info *dataf;
{
    ...
    struct source_field_info *srsf=dataf->srsf, *fof_find_srs_fld();
    struct source_field_info *fof_check_srs_fld();
    char *field_name=dataf->field_name;
    struct field_derivation_info *derf;
    struct data_field_info *datafx, *datafy, *fof_insert_data_field();
    struct data_field_info *fof_remove_data_field();
    struct data_field_info *fof_data_field_spair();
    struct data_field_info *fof_push_data_field();
    struct data_field_info *fof_que_data_field();
	 ...
	 */
	    ...
	    if(!(datafx = fof_duplicate_field("BZ"))) {
		datafx = fof_establish_data_field("BZ");
	    }
	    dataf->src_fld = datafx;

	    if(!(datafx = fof_duplicate_field("SN"))) {
		datafx = fof_establish_data_field("SN");
	    }
	...
	if(!(datafx = fof_duplicate_field("LU"))) {
	    datafx = fof_establish_data_field("LU"); /* unfolded linear power */
	}
	dataf->src_fld = dataf->thr_fld = datafx;
	dataf->proc_ptr = fof_gen_dbz_field;
	dataf->threshold = dataf->srsf->cal_info->power_threshold;
	...
	if(strstr("BS.BF", dataf->srsf->field_name)) {
		...
		datafx = fof_establish_data_field("LU");
	    }
	    dataf->src_fld = datafx;
	...
	/* the log channel SNR */
	if(!(datafx = fof_duplicate_field("DM"))) {
	    datafx = fof_establish_data_field("DM");
	}
	dataf->src_fld = datafx;
	dataf->proc_ptr = fof_gen_snr;
	fof_que_data_field(dataf);
	...
	if(MATCH2("BW", dataf->srsf->field_name)) {
	    ...
	    if(!(datafx = fof_duplicate_field("NC"))) {
		datafx = fof_establish_data_field("NC");
	    }
	...
	if(srsf = fof_check_srs_fld("NC", &derf)) { /* NCP! */
	    if(!(datafx = fof_duplicate_field("NC"))) {
		datafx = fof_establish_data_field("NC");
	    }
	...
	else if((srsf = fof_find_srs_fld("SR")) && srsf->present) {
	    /* raw spectral width! */
	    if(!(datafx = fof_duplicate_field("SR"))) {
		datafx = fof_establish_data_field("SR");
	    }
	    dataf->threshold = datafx->srsf->sysatts->sw_threshold;
	    dataf->thr_fld = datafx;
	    dataf->hi_threshold = YES;
	}
	else if(srsf = fof_check_srs_fld("SN", &derf)) { /* SNR! */
	    if(!(datafx = fof_duplicate_field("SN"))) {
		datafx = fof_establish_data_field("SN");
	    }
	...
	if(MATCH2("BN", dataf->srsf->field_name)) {
	    ...
	    if(!(datafx = fof_duplicate_field("RT"))) {
		datafx = fof_establish_data_field("RT");
	    }
	    ...
	    /* the denomenater
	     */
	    if(!(datafx = fof_duplicate_field("NR"))) {
		datafx = fof_establish_data_field("NR");
	    }
	    dataf->src_fldb = datafx;
	}
	...
	if(!(datafx = fof_duplicate_field("XM"))) {
	    datafx = fof_establish_data_field("XM");
	}
	dataf->proc_ptr = fof_gen_ldr_field;
	dataf->src_flda = datafx;

	if(!(datafx = fof_duplicate_field("XU"))) {
	    datafx = fof_establish_data_field("XU");
	}
	dataf->src_fldb = datafx;
	fof_que_data_field(dataf);
    }
	...
	if(!(datafx = fof_duplicate_field("ZR"))) { /* raw zdr */
	    datafx = fof_establish_data_field("ZR");
	}
	dataf->src_fld = datafx;

	if(!(datafx = fof_duplicate_field("WP"))) {
	    datafx = fof_establish_data_field("WP");
	}
	dataf->src_flda = datafx;
	/* then setup the reciprocal of ZR in watts
	 */
	if(!(datafx = fof_duplicate_field("ZW"))) {
	    datafx = fof_establish_data_field("ZW");
	}
	dataf->src_fldb = datafx;
	fof_que_data_field(dataf);
    }
...
void fof_stack_ray_que_spairs(this)
  struct fof_ray_que *this;
{
    ...
    if(!this)
	  return;
    this->last->next = ray_que_spairs;
    ray_que_spairs = this;
}
/* c------------------------------------------------------------------------ */

void fof_stack_sweep_que_spairs(this)
  struct fof_sweep_que *this;
{
    if(!this)
	  return;
    this->last->next = sweep_que_spairs;
    sweep_que_spairs = this;
}
/* c------------------------------------------------------------------------ */

struct fof_sweep_que *
fof_sweep_que_spair(top)
  struct fof_sweep_que **top;
{
    struct fof_sweep_que *this=sweep_que_spairs;

    if(!this) {
	if(!(this = SMALLOC(struct fof_sweep_que))) {
	    printf("Could not allocate another sweep_que_spair\n");
	    exit(1);
	}
	sweep_que_count++;
	memset(this, 0, sizeof(struct fof_sweep_que));
	sweep_que_spairs = this;
    }
	...
	*top = this;
    ...
    this->next = NULL;
    this->first_ray = NULL;
    this->daz_sum = this->del_sum = this->dfx_sum = this->fxd_sum = 0;
    this->drot_sum = 0;
    this->ray_count = 0;
    return(this);
}
...
int zatt_arg_ptrs(att, string_space, str_ptrs)
  char *att, *string_space, **str_ptrs;
{
...
int xnext_att(fp, att)		/* get the next attribute */
  FILE *fp;
  char *att;
{
    int ch, nn=0;
...
int znext_att(fp, att)		/* get the next attribute */
  FILE *fp;
  char *att;
{
    int ch, nn=0;
...
void ctypef( ff, m, n )	/* type as floats */
  float *ff;
  int m, n;
{
    int ii=m, s=0, nn=n;
