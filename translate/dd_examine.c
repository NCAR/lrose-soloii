  ...
  char *argv[];
{
    ...
    char *aa, *b, *c, *getenv(), *sdev;
    struct input_read_info *iri, *dd_return_ios();
    double d, fabs();
...
# endif
	    printf("\n");
	    nn = nn < 200 ? nn : 200;
	    ctypeu16((unsigned short *)iri->top->buf, 0, nn);
# ifdef obsolete
	    printf("\n");
...
# else
	    slm_ctypeu16(slm, iri->top->buf, view_char, view_char_count);
# endif
	}
    }
    else if(ival == 3) {
	...
	if(ddx_start_stop_chars() >= 0) {
	    printf("\n");
	    slm_xctypeu16(slm, iri->top->buf, view_char, view_char_count);
	}
    }
    else if(ival == 7) {
    }
...
ddx_dorade_bkwd_desc_loop(aa, ii, jj, offset, sizeof_file)
  char *aa;
  int ii, jj;
  long offset, sizeof_file;
{
...
ddx_dorade_fwd_desc_loop(aa, ii, jj, offset, sizeof_file)
  char *aa;
  int ii, jj;
  long offset, sizeof_file;
{
...
char *dd_print_seds(gd, str)
  struct generic_descriptor *gd;
  char *str;
{
    char *a=str, *b,  c[16], *str_terminate();
    int ii, nn;
    unsigned int gdsos = gd->sizeof_struct;

    if(gottaSwap) { swack4(&gd->sizeof_struct, &gdsos); }
...
#ifndef notyet
    char *dd_print_frib();
    char *dd_print_frib_v0();
    char *dd_print_frib_v1();
#endif
    char *dd_print_parm();
    char *dd_print_radd();
    char *dd_print_rdat();
...
char *dd_print_ndds(ndds, str)
  struct nav_descript *ndds;
  char *str;
{
   if(gottaSwap) {
...
char *dd_print_situ(situ, str)
  struct insitu_descript *situ;
  char *str;
{
    int ii, nn;
    char name[16], units[16], *str_terminate();
...
char *dd_print_isit(isit, str)
  struct insitu_data *isit;
  char *str;
{
    int ii, nn;
    float *fptr;
...
char *dd_print_time(times, str)
  struct time_series *times;
  char *str;
{
    int ii, nn;
    float *fptr;
  ...
  char *str;
{
   ...
   if(gottaSwap) {
      ddin_crack_rktb (rktb, swapBuf, (int)0);
...
char *dd_print_comm(comm, str)
  struct comment_d *comm;
  char *str;
{
    unsigned int gdsos = comm->comment_des_length;

    if(gottaSwap) { swack4(&comm->comment_des_length, &gdsos); }
...
char *dd_print_asib(asib, str)
  struct platform_i *asib;
  char *str;
{
   if(gottaSwap) {
      ddin_crack_asib (asib, swapBuf, (int)0);
  ...
  char *str;
{
   ...
   if(gottaSwap) {
      swack4(&celv->dist_cells[0], f);
      swack4(&celv->dist_cells[1], f+1);
      swack4(&celv->dist_cells[2], f+2);

      /* we must swap "number_cells" before we use it */
      swack4(&celv->number_cells, &numCells);

      swack4(&celv->dist_cells[numCells-1], f+3);
      ddin_crack_celv (celv, swapBuf, (int)0);
...
char *dd_print_cfac(cfac, str)
  struct correction_d *cfac;
  char *str;
{
   if(gottaSwap) {
      ddin_crack_cfac (cfac, swapBuf, (int)0);
  ...
  char *str;
{
   ...
   if(gottaSwap) {
      eld_crack_cspd (cspd, swapBuf, (int)0);
...
char *dd_print_csfd(csfd, str)
  struct cell_spacing_fp_d *csfd;
  char *str;
{
    int i;
    char *a=str;
  ...
  char *str;
{
   ...
   if(gottaSwap) {
      eld_crack_frad (frad, swapBuf, (int)0);
...
#ifndef notyet
char *dd_print_frib(a, str)
  char *a, *str;
{
    struct generic_descriptor *gd;
    char *b, *dd_print_frib_v0(), *dd_print_frib_v1();

    gd = (struct generic_descriptor *)a;
    if(gd->sizeof_struct == sizeof(struct field_radar_i_v0)) {
	b = dd_print_frib_v0(a, str);
    }
    else {
	b = dd_print_frib_v1(a, str);
    }
}
/* c------------------------------------------------------------------------ */

char *dd_print_frib_v1(frib, str)
  struct field_radar_i *frib;
  char *str;
{
    int i, nn;
    char *a=str, c[128], *str_terminate();
...
char *dd_print_frib_v0(frib, str)
  struct field_radar_i_v0 *frib;
  char *str;
{
    int i;
    char *a=str, c[128], *str_terminate();
  ...
  char *str;
{
   ...
   if(gottaSwap) {
      ddin_crack_parm (parm, swapBuf, (int)0);
  ...
  char *str;
{
   ...
   if(gottaSwap) {
      ddin_crack_radd (radd, swapBuf, (int)0);
  ...
  char *str;
{
   ...
   if(gottaSwap) {
      ddin_crack_qdat (rdat, swapBuf, (int)0);
...
char *dd_print_ryib(ryib, str)
  struct ray_i *ryib;
  char *str;
{
   if(gottaSwap) {
      ddin_crack_ryib (ryib, swapBuf, (int)0);
...
char *dd_print_sswb(sswb, str)
  struct super_SWIB *sswb;
  char *str;
{
    char *a=str,  c[16], *str_terminate();
    int ii;

   if(gottaSwap) {
      if(LittleEndian) { ddin_crack_sswbLE (sswb, swapBuf, (int)0); }
      else { ddin_crack_sswb (sswb, swapBuf, (int)0); }
  ...
  char *str;
{
   ...
   if(gottaSwap) {
      ddin_crack_swib (swib, swapBuf, (int)0);
...
char *dd_print_vold(vold, str)
  struct volume_d *vold;
  char *str;
{
    char *a=str, c[64], d[12], e[12], *str_terminate();

   if(gottaSwap) {
      ddin_crack_vold (vold, swapBuf, (int)0);
...
char *dd_print_wave(wave, str)
  struct waveform_d *wave;
  char *str;
{
    int i;
    char *a=str, c[128], d[128], *str_terminate();
