...
void type5_new()
{
    ...
    struct type5_stuff *t5s=hs->t5s->next;
    float *fptr, *fus;
    double d, pow(), exp(), log(), T, Te, Ee, AlogEe, d_time_stamp();
...
# define   MASK7 0x7f
	...
	if((ival >> 8) & MASK23) {
	     ...
	     */
	    fraction = ((long)0xff000000 | (ival >> 8)) *
...
int hrdstd_read(fid, buf, size, io_type)
  int fid, size, io_type;
  char *buf;
{
    static int count=0, t_count=0, std_fid=-1;
    int ii, nn, mark;
    char *a, *get_tagged_string(), *getenv();
